...
async function publishInteractiveCardViaLarkCli(publish) {
  ...
  const larkAs = process.env.LARK_CLI_AS || "bot";
  const command = [larkCli];
  if (process.env.LARK_CLI_PROFILE) {
    command.push("--profile", process.env.LARK_CLI_PROFILE);
  }
  command.push("im");
  ...
  const result = await runCommand(command, {
    timeoutSeconds: process.env.FEISHU_CARD_TIMEOUT_SECONDS || 2,
  });
  return {
    ...result,
...
function interactiveCardIdempotencyKey(publish) {
  const raw = [
    "openclaw-card",
    publish.reply_to || "",
    publish.chat_id || "",
...
function buildCardDeliveryFailureFallback(cardDelivery) {
...
function buildRouterFailureFallback(error) {
  return [
