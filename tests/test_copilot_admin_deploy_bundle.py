...
class CopilotAdminDeployBundleTest(unittest.TestCase):
    def test_deploy_bundle_is_staging_ready_but_production_blocked(self) -> None:
        ...
        self.assertIn("audit_readonly_gate", {check["name"] for check in result["checks"]})
        self.assertIn("launch_evidence_bundle_export", {check["name"] for check in result["checks"]})
