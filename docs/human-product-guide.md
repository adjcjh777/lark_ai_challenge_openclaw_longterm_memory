...
- OpenClaw Agent runtime 已有受控证据，但不是 first-class 原生工具注册。
...
- `memory.*` 已注册为 OpenClaw first-class 原生工具。
- 完整多租户后台。
- 审计 UI、管理员配置和长期运维。
- 长期 embedding 服务。
...
当前仓库已经实现了中间的 Copilot Core、schema、benchmark、demo 和受控接入证据；上线前还要补 OpenClaw first-class 工具、Feishu websocket running、真实权限和生产运维。
...
1. 把 `memory.*` 注册为 OpenClaw first-class 原生工具。
2. 补 OpenClaw Feishu websocket running 证据。
3. 把 demo 权限模型升级为真实飞书权限映射。
4. 做生产存储、索引、迁移和审计查询方案。
5. 扩大真实 Feishu ingestion，但继续坚持 candidate-only。
