...
- OpenClaw Feishu websocket running 已有本机 staging 证据，但真实 DM 还未补齐“飞书消息 -> OpenClaw Agent -> 本项目 `fmc_*` 工具 -> `memory.*` / `CopilotService`”的 live E2E 证据。
- Limited Feishu ingestion 本地底座已支持群聊、文档、任务、会议、Bitable 来源文本进入 candidate-only pipeline。
...
|---|---|---|---|---|
| 搜索 | `上次定的生产部署 region 是哪个？` | 当前 active 结论、证据 quote、下一步动作；审计详情放在底部。 | 没找到时提示补充主题或先创建候选。 | 不展示 superseded 旧值或 raw events。 |
| 候选确认 | `记住：生产部署必须加 --canary，region 用 ap-shanghai。` -> `确认这条` | 先生成 candidate，再确认最近 candidate；用户不用输入内部 ID。 | 上下文不明确时提示 `/confirm <candidate_id>`。 | 真实飞书来源不能自动 active。 |
...
| 候选审核卡 | 新记忆、来源、证据、风险、是否冲突。 | reviewer / owner / admin 才能看到 `确认保存` 和 `拒绝候选`；点击后仍走 `handle_tool_request()` / `CopilotService`。 | 权限不足、permission 缺失或畸形时 fail closed，并隐藏未授权内容。 |
...
这些卡片仍属于 demo / sandbox / pre-production 验收面，不代表生产部署、全量飞书空间接入或真实 Feishu DM 到本项目 `fmc_*` / `memory.*` 链路的 live E2E 已完成。
...
1. 把真实飞书消息进入 OpenClaw Agent 后自然选择本项目 first-class `fmc_*` 工具、再进入 `memory.*` / `CopilotService` 的 live E2E 证据补齐。
