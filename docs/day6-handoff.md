...
今天主要做了两件事：第一，把机器人回复从普通文字升级成飞书卡片；第二，把“历史决策”“矛盾更新”“版本链”“人工确认”这些场景都整理成更适合截图给评委看的样子。

你今晚不用看代码，先按下面的“队友今晚任务”做检查就行。重点是：卡片文案评委能不能一眼看懂，截图够不够支撑 5 分钟 Demo。

做对的标准：

- 看到卡片时，能马上知道“当前有效结论是什么”。
- 能看出旧规则已经被新规则覆盖。
- 看不到完整 token、内部链接或敏感信息。
- 截图能直接放进白皮书、报告或答辩材料里。

如果卡住，把飞书截图、你输入的命令、机器人回复原文发给我；不用自己改核心代码。
...
1. 打开飞书测试群，看机器人卡片是否像“项目结论卡”，而不是一段聊天记录。重点看：结论、版本、来源、是否被覆盖。
2. 补齐 4 张截图：查历史决策、改规则后的矛盾更新、版本链、人工确认队列。`拒绝候选 1` 和版本链截图已经有了，优先补缺的。
3. 打开 `docs/day6-scope-adjustment.md`，看“初赛要做”和“复赛再做”的边界是否太贪心。如果觉得太多，直接在文档里标注“建议挪到复赛”。
4. 如果 4 月 29 日主题直播提了新要求，在本 handoff 后面追加“直播后复核”小节，写清新要求和它影响哪一块。

今晚不用做：

- 不用改飞书 token、`.env` 或真实权限。
- 不用改卡片发送代码。
- 不用处理 Hermes Agent 参考文档。
