...
3. 你今晚要交付：整理一组评委能看懂的截图，并把不顺口的回复文案记到 `docs/day3-qa-notes.md`。
4. 做对的标准：截图里能看出“先记住 v1、再覆盖成 v2、最后召回只看到 v2”。
5. 遇到问题发我：发测试群截图、机器人启动日志、以及你卡住的是哪一步。
...
3. 检查机器人回复是否一眼能懂；不懂的地方先写到 `docs/day3-qa-notes.md`，不用直接改代码。
4. 给白皮书补一段白话解释：普通搜索只能找文字；记忆引擎能知道哪条是当前有效版本、来源在哪里、旧版本为什么被覆盖。

今晚不用做：

- 不用改 `.env`、飞书 token、群聊 ID 或数据库文件。
- 不用调并发、多用户或卡片按钮。
- 不用改 Bitable，同步看板是 D4 的任务。
