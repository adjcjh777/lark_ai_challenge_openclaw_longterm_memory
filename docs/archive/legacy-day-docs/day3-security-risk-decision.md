...
外部分工获取真实值的步骤已写入 `docs/reference/local-lark-cli-setup.md`，真实值只应放在本机 `.env.local`。
...
- Git 历史：更早的外部分工配置文档提交中出现过应用 `App ID`。
...
暂不自动执行 Git 历史重写，因为 history rewrite + force push 会改变远端提交图，可能影响外部分工已有 clone、分支和未同步工作。
...
4. force push 后通知外部分工重新 fetch / rebase 或重新 clone。
