...
状态：历史主控计划 + 产品化参考；2026-05-05 及以前 implementation plan 已全部完成，不再作为后续执行入口
...
> **当前执行入口更新（2026-04-28）**：2026-05-05 及以前的日期计划事实上已经完成；这些 implementation plan 只保留为历史计划、验收证据和风险参考，不再需要继续执行。后续目标已经升级为完整可用 Copilot，当前入口是 `docs/productization/full-copilot-next-execution-doc.md`，下一步优先 Phase A：Storage Migration + Audit Table。
...
- 下面的 2026-04-27 启动描述是历史上下文，不再作为当前日期判断。
- 总控文档原时间线中 2026-04-26 对应 D3，但 PRD 已经把路线调整为从 2026-04-26 起一周完成 Feishu Memory Copilot MVP。
- 2026-05-05 及以前任务已经闭环；后续执行以完整可用 OpenClaw-native Copilot 产品化为主线。
- 旧 Day1-Day7 产物和 2026-05-05 及以前日期计划只作为参考资产、验收证据和兜底能力。
...
本节保留为历史启动 Prompt 参考。2026-05-05 及以前的 implementation plan 已全部完成，不再需要按“当天计划”继续执行。后续新对话如果要推进项目，应优先使用 `docs/productization/full-copilot-next-execution-doc.md` 中的完整可用 Copilot 执行提示词。

如果任务目标是继续推进完整产品，请优先读取：

1. `docs/productization/full-copilot-next-execution-doc.md`
2. `docs/productization/prd-completion-audit-and-gap-tasks.md`
3. `docs/productization/complete-product-roadmap-prd.md`
4. `docs/productization/complete-product-roadmap-test-spec.md`
5. 当前产品化阶段直接相关的 contract / runbook / handoff

当前阶段的安全顺序是：Phase A Storage Migration + Audit Table -> Phase B 真实 OpenClaw Agent Runtime 验收 -> Phase C Feishu Staging Runbook -> Phase D Live Cognee/Ollama Embedding Gate -> Phase E Product QA。不要再回头执行 2026-05-05 及以前的旧日期计划。
...
下面这段是旧日期计划时期的启动 Prompt，保留是为了复盘 2026-04-26 至 2026-05-05 的执行方式。后续不要直接复制它来启动当前主线；要推进完整可用 Copilot，请改用 `docs/productization/full-copilot-next-execution-doc.md`。

```text
工作目录：/Users/junhaocheng/feishu_ai_challenge

请复盘某个历史日期的 Feishu Memory Copilot 任务。先判断该日期计划是否已经完成，再决定只总结证据、补齐遗漏记录或更新看板；不要把 2026-05-05 及以前日期计划当作当前待执行任务。

必须先读取并遵循：
1. AGENTS.md
2. docs/productization/full-copilot-next-execution-doc.md
3. docs/feishu-memory-copilot-implementation-plan.md（历史参考）
4. 仅在复盘绝对日期任务时读取对应 docs/plans/YYYY-MM-DD-implementation-plan.md
5. 如果复盘日期存在 handoff，也读取对应 docs/plans/YYYY-MM-DD-handoff.md
6. README.md 顶部“今天先做这个：我的任务”
7. git status --short
8. 当前代码结构，尤其是 memory_engine/、memory_engine/copilot/、agent_adapters/openclaw/、benchmarks/、tests/、docs/

启动前先做状态确认：
- 复述当前绝对日期、产品化阶段、目标、“做到什么程度”、本阶段不做和补充任务。
- 检查 OpenClaw 版本锁，运行：python3 scripts/check_openclaw_version.py
- 检查工作树状态，不要覆盖用户或历史未跟踪文件。
- 检查飞书共享看板是否需要创建或更新程俊豪主线任务和补充任务；如需同步，直接按 AGENTS.md 用 lark-cli 更新并读回确认。
- 如果发现历史日期任务已完成，不要重复写代码；先总结已完成证据，并只处理用户最新要求或计划中仍未闭环的事项。

执行要求：
- 当前产品化任务严格按 `docs/productization/full-copilot-next-execution-doc.md` 推进。
- 2026-05-05 及以前日期计划已经完成，只能作为历史证据和风险参考。
...
- 如果改了 Copilot schema/tools/retrieval/benchmark，追加当前产品化阶段指定的 unittest 或 copilot_* benchmark。
...
### 指定日期任务复盘 Prompt（仅供历史补漏）

如果要复盘某一天，或需要补一条历史记录，复制下面这段并把 `YYYY-MM-DD` 改成目标日期，例如 `2026-04-28`。2026-05-05 及以前任务默认视为已完成；不要用这段启动当前产品化主线。

```text
工作目录：/Users/junhaocheng/feishu_ai_challenge
目标日期：YYYY-MM-DD

请复盘目标日期对应的 Feishu Memory Copilot 每日任务。先判断该日期任务是否已经完成，再决定只总结证据、补齐遗漏记录或更新看板；不要重复实现已经完成的历史任务。

必须先读取并遵循：
1. AGENTS.md
2. docs/productization/full-copilot-next-execution-doc.md
3. docs/feishu-memory-copilot-implementation-plan.md（历史参考）
4. docs/plans/YYYY-MM-DD-implementation-plan.md（仅用于历史复盘）
5. 如果存在上一日 handoff 或该日期 handoff，也读取对应 docs/plans/YYYY-MM-DD-handoff.md
6. README.md 顶部“今天先做这个：我的任务”
7. git status --short
8. 当前代码结构，尤其是 memory_engine/、memory_engine/copilot/、agent_adapters/openclaw/、benchmarks/、tests/、docs/
...
- 2026-05-05 及以前日期计划已完成，不再按“今日执行清单”推进；只允许补齐遗漏记录或用户明确指定的修正。
- 新功能优先进入 memory_engine/copilot/ 和 agent_adapters/openclaw/。
- 不要从大改 memory_engine/repository.py、memory_engine/feishu_runtime.py 或旧 CLI/Bot handler 开始。
...
如果只想检查历史计划是否足够细，可以复制这一段。当前产品化任务应优先使用 `docs/productization/full-copilot-next-execution-doc.md`。

```text
工作目录：/Users/junhaocheng/feishu_ai_challenge

请只复核指定历史日期的 implementation plan，不要开始实现代码。

必须先读取：
1. AGENTS.md
2. docs/productization/full-copilot-next-execution-doc.md
3. docs/feishu-memory-copilot-implementation-plan.md（历史参考）
4. 指定历史日期的 docs/plans/YYYY-MM-DD-implementation-plan.md
...
本轮 MVP 目标结构如下，保留为历史目标目录参考。当前产品化执行应优先读取 `docs/productization/full-copilot-next-execution-doc.md` 和相关 contracts / handoff，再按当前阶段创建或修改对应代码文件。
...
本主控计划只保留历史执行顺序和边界；2026-05-05 及以前具体代码实现已经按日期任务完成。后续具体代码实现按 `docs/productization/full-copilot-next-execution-doc.md` 和当前产品化阶段 handoff 推进。

下一步如果用户明确要求“开始执行”，不要再从旧 MVP schema skeleton 开始。当前建议先读 `docs/productization/full-copilot-next-execution-doc.md` 的 Phase A，并从以下产品化缺口进入：

1. `memory_engine/db.py`、`memory_engine/repository.py`：Storage Migration + Audit Table。
2. `memory_engine/copilot/service.py`、`memory_engine/copilot/governance.py`、`memory_engine/copilot/permissions.py`：把审计、权限和状态机写成同一条事实链。
3. `memory_engine/copilot/healthcheck.py`、`scripts/check_copilot_health.py`：把 migration/audit 检查纳入可诊断入口。
4. `tests/test_copilot_permissions.py`、`tests/test_copilot_healthcheck.py`：先用测试锁住 permission fail-closed 和 audit record。
5. `docs/productization/contracts/storage-contract.md`、`docs/productization/contracts/audit-observability-contract.md`：同步记录 schema 和验收口径。

`memory_engine/repository.py` 只允许围绕 Phase A migration/audit 做小步、可测试的扩展；不要顺手重写旧 repository 或旧 `memory_engine/feishu_runtime.py`。
