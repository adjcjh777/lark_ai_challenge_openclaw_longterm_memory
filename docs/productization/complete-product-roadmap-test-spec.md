...
当前状态：permission fail-closed 和 OpenClaw live bridge 已完成，见 commit `cb21bc7`。Phase B runtime evidence 已补：OpenClaw Agent run `b252f11e-b49d-495c-a14f-0b823a888a5e` 通过 `exec` 调用证据脚本，三条 Copilot flow 全部 `ok=true`。后续 first-class OpenClaw 原生工具注册、Agent 本地 `fmc_*` 工具调用验证、Feishu websocket staging running 证据和一次受控真实 Feishu DM `fmc_memory_search` allow-path 证据已补。本阶段仍不是生产 Feishu live ingestion；真实 Feishu DM 稳定长期路由仍需继续验收。
