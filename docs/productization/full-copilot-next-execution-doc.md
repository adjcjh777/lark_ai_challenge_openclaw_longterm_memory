...
还不能说完成的是：生产全量 workspace ingestion、生产 DB / 生产监控 / productized live 长期运行、完整多租户企业后台、自然出现的企业级大样本覆盖、长期调度和 rate-limit 姿态。受控 normal Sheet 和真实群消息已经足够关闭当前 controlled readiness gate；后续产品化仍要扩到更多 organic 项目/企业 Sheet、Docs、Base、Wiki space，以及更长时间运行样本。2026-05-04 新增的 `scripts/check_workspace_productized_ingestion_readiness.py` 是更严格的 production/full-workspace gate：只有非示例 manifest 同时证明 source coverage、scheduler/cursor、rate-limit/backoff、governance、operations 和 24h+ long-run，才允许 `goal_complete=true`。
...
4. 需要宣称“全量 workspace 接入”前，先填充并审计 `deploy/workspace-ingestion.production-evidence.example.json` 的非示例 manifest，再跑 `python3 scripts/check_workspace_productized_ingestion_readiness.py --manifest <manifest> --require-productized-ready --json`。
...
- productized workspace ingestion：已有 production evidence gate 和 example manifest，但没有非示例 manifest 证明 organic source coverage、scheduler/cursor、rate-limit/backoff、governance、operations 和 24h+ long-run，因此不能说全量 workspace ingestion 完成。
