...
当前 workspace pilot 已有这些证据：Drive root / folder walk、Wiki space walk、显式 `--resource type:token[:title]`、doc/docx/wiki/sheet/bitable 类型路由、source registry / run registry、revision 去重、unchanged skip、同 filter stale 标记、failed fetch 记录、discovery cursor、registry gate、mixed-source 本地佐证 gate、warm-path latency gate、真实 lark-cli fetch latency gate、真实 docx + Bitable 多来源 temp-DB gate、真实 chat log + workspace resource gate、真实同结论 gate、支持 Drive/Wiki 只读扩搜的真实样本匹配发现器、总 readiness gate 和项目 normal Sheet 只读证据 gate。早期广泛只读扩搜证明当前账号没有自然出现的项目 normal Sheet 和同结论样本；用户明确批准后创建受控 normal Sheet，并由一条真实 OpenClaw Feishu 群消息复述同一 durable fact。随后 `check_workspace_project_sheet_evidence_gate.py` 读回 `eligible_project_normal_sheet_count=1`；`check_workspace_real_same_conclusion_sample_finder.py` 读回 `same_fact_match_count=1`，strict gate source types 为 `feishu_message` 和 `lark_sheet`；最终 `check_workspace_ingestion_goal_readiness.py` 返回 `goal_complete=true`、`status=pass`、`failures=[]`。2026-05-04 又补了一次 bot 单聊回读：`/recall Workspace ingestion readiness gate Sheet 样本` 通过 OpenClaw first-class router 进入 `fmc_memory_search`，机器人卡片第一条 active 记忆就是受控 Sheet 的 reviewed normal Sheet 结论，并带 `lark_sheet` 证据、`request_id` / `trace_id` 和 `permission: allow / scope_access_granted` 审计。最近本地 warm-path gate 读回平均 5.51ms、最大 5.599ms；retrieval fallback 已在单次请求内复用 active-memory index 和 curated vector scores，trace 里记录 structured / keyword / vector / Cognee / rerank 的 stage-level `elapsed_ms`；lark-cli wrapper 对成功、解析失败、非零返回、timeout、CLI 缺失和异常路径都记录 `elapsed_ms`；workspace source metadata 也记录 document fetch、Sheet info/read、Bitable record-get timing，方便把本地热路径、子进程边界和飞书网络耗时分开看。这些 timing 和 live readback 是 limited pilot 的诊断、防回退和受控验收证据，不是生产 SLO。

还不能说完成的是：生产全量 workspace ingestion、生产 DB / 生产监控 / productized live 长期运行、完整多租户企业后台、自然出现的企业级大样本覆盖、长期调度和 rate-limit 姿态。受控 normal Sheet 和真实群消息已经足够关闭当前 controlled readiness gate；后续产品化仍要扩到更多 organic 项目/企业 Sheet、Docs、Base、Wiki space，以及更长时间运行样本。2026-05-04 新增的 `scripts/check_workspace_productized_ingestion_readiness.py` 是更严格的 production/full-workspace gate：只有非示例 manifest 同时证明 source coverage、scheduler/cursor、rate-limit/backoff、governance、operations 和 24h+ long-run，才允许 `goal_complete=true`。`scripts/check_workspace_ingestion_objective_completion.py` 会把用户 1-6 项目标映射到 artifact checklist，并把最终完成判定绑定到这个 productized gate。同日新增的 `scripts/run_workspace_ingestion_schedule.py` 是 one-shot 调度入口，默认 plan-only；`scripts/sample_workspace_ingestion_schedule.py` 可连续写出脱敏 schedule reports；`scripts/collect_workspace_ingestion_long_run_evidence.py` 可把多份脱敏 report 汇总成 production manifest patch；`scripts/collect_workspace_source_coverage_evidence.py` 可把已有真实 gate / workspace ingest / schedule result 报告归一化为 source coverage patch；`scripts/collect_workspace_ops_governance_evidence.py` 可把外部 rate-limit、governance 和 operations 证明归一化为三个 manifest patch section；`scripts/merge_workspace_productized_ingestion_evidence.py` 可把多个 patch 合并成 manifest 并立即跑 gate。最新隔离 DB 非 dry-run sampler 已跑 4 次真实 schedule execute，fetch/candidate pipeline 均 pass，source coverage collector 读回 `document_feishu=8`、`lark_bitable=43`；registry 现在解析 Drive/Wiki `result_meta.icon_info.version`，同一批资源复跑会跳过 7 个 unchanged fetch，`discovery_and_cursoring` 在 partial manifest 中已 pass。但 `lark_doc`、`lark_sheet`、`wiki`、same-conclusion、conflict-negative 仍缺，long-run collector 也正确 blocked：`successful_run_count=4`、`window_hours=0.191`。这不是 24h+ 长跑完成证据。
...
8. 多轮 schedule report 只保留脱敏输出，再用 `python3 scripts/collect_workspace_ingestion_long_run_evidence.py --schedule-report-glob '<redacted-dir>/schedule-report-*.json' --evidence-ref <redacted-ref> --json` 生成 long-run manifest patch。
...
10. 需要宣称“全量 workspace 接入”前，先填充并审计 `deploy/workspace-ingestion.production-evidence.example.json` 的非示例 manifest，再跑 `python3 scripts/check_workspace_productized_ingestion_readiness.py --manifest <manifest> --require-productized-ready --json` 和 `python3 scripts/check_workspace_ingestion_objective_completion.py --manifest <manifest> --json`。
...
- productized workspace ingestion：已有 one-shot schedule runner、schedule sampler、source coverage collector、ops/governance/rate-limit collector、long-run evidence collector、evidence patch merger、production evidence gate、objective completion audit 和 example manifests；最新非 dry-run sampler 已证明一小批 organic docx/Base 能真实进入 candidate pipeline，并让 scheduler/cursor 子项通过，但没有非示例 manifest 完整证明 `lark_doc`、`lark_sheet`、`wiki`、same-conclusion、conflict-negative、rate-limit/backoff、governance、operations 和 24h+ long-run，因此不能说全量 workspace ingestion 完成。
