...
- Feishu 群图谱节点发现：Feishu live 入口会在 allowlist 判断前把群登记为同 tenant/org 下的 `feishu_chat` 节点，并建立 organization -> chat 边；未在 allowlist 的新群只登记最小群元数据，不写 raw_events、不创建 candidate、不回复消息。详见 [Feishu 群图谱节点 handoff](handoffs/feishu-group-graph-node-handoff.md)。这不是全量 workspace ingestion，也不是生产级长期图谱服务。
