...
- 九项剩余任务 completion audit gate：2026-05-02 新增 `scripts/check_openclaw_feishu_productization_completion.py --json`，把非 @ 群消息 live 投递、单监听、first-class `fmc_*` routing、第二真实用户权限负例、真实 `/review` DM/card、Dashboard auth、clean demo DB、Cognee/embedding long-run 和 no-overclaim 逐项映射到 artifact 或 live evidence gate。2026-05-03 传入 Cognee long-run evidence 后 item 8 pass，但 `goal_complete=false`，blockers 仍是非 @ 群消息 live 投递、first-class 工具扩样、第二真实用户拒绝和真实 `/review` DM/card E2E。后续真实扩样可先用 `scripts/prepare_feishu_live_evidence_run.py` 生成单监听 preflight 和日志路径，再用 `scripts/collect_feishu_live_evidence_packet.py` 生成 sanitized packet，并通过 `--feishu-live-evidence-packet` 输入 completion audit；该 packet 仍要求每个底层 gate 的 exact pass reason。
