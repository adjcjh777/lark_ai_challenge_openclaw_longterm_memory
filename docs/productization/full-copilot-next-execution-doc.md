...
还不能说完成的是：生产全量 workspace ingestion、生产 DB / 生产监控 / productized live 长期运行、完整多租户企业后台、自然出现的企业级大样本覆盖、长期调度和 rate-limit 姿态。受控 normal Sheet 和真实群消息已经足够关闭当前 controlled readiness gate；后续产品化仍要扩到更多 organic 项目/企业 Sheet、Docs、Base、Wiki space，以及更长时间运行样本。2026-05-04 新增的 `scripts/check_workspace_productized_ingestion_readiness.py` 是更严格的 production/full-workspace gate：只有非示例 manifest 同时证明 source coverage、scheduler/cursor、rate-limit/backoff、governance、operations 和 24h+ long-run，才允许 `goal_complete=true`。同日新增的 `scripts/run_workspace_ingestion_schedule.py` 是 one-shot 调度入口，默认 plan-only；`scripts/sample_workspace_ingestion_schedule.py` 可连续写出脱敏 schedule reports；`scripts/collect_workspace_ingestion_long_run_evidence.py` 可把多份脱敏 report 汇总成 production manifest patch；`scripts/collect_workspace_source_coverage_evidence.py` 可把已有真实 gate / workspace ingest / schedule result 报告归一化为 source coverage patch；`scripts/merge_workspace_productized_ingestion_evidence.py` 可把多个 patch 合并成 manifest 并立即跑 gate。当前只有一次 dry-run report，long-run collector 正确 blocked：`successful_run_count=1`、`window_hours=0`，这不是长跑完成证据。
...
7. 多轮 schedule report 只保留脱敏输出，再用 `python3 scripts/collect_workspace_ingestion_long_run_evidence.py --schedule-report-glob '<redacted-dir>/schedule-report-*.json' --evidence-ref <redacted-ref> --json` 生成 long-run manifest patch。
8. 多个 evidence patch 准备好后，用 `python3 scripts/merge_workspace_productized_ingestion_evidence.py --patch <patch.json> --output <manifest.json> --json` 合并并验证；合并器会拒绝未知 section、placeholder 和 secret-like 值。
9. 需要宣称“全量 workspace 接入”前，先填充并审计 `deploy/workspace-ingestion.production-evidence.example.json` 的非示例 manifest，再跑 `python3 scripts/check_workspace_productized_ingestion_readiness.py --manifest <manifest> --require-productized-ready --json`。
...
- productized workspace ingestion：已有 one-shot schedule runner、schedule sampler、source coverage collector、long-run evidence collector、evidence patch merger、production evidence gate 和 example manifests，但没有非示例 manifest 证明 organic source coverage、真实多轮 scheduler/cursor 运行、rate-limit/backoff、governance、operations 和 24h+ long-run，因此不能说全量 workspace ingestion 完成。
