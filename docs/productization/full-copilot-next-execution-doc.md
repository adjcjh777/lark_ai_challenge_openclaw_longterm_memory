...
还不能说完成的是：生产全量 workspace ingestion、生产 DB / 生产监控 / productized live 长期运行、完整多租户企业后台和长期运行 SLO。受控 normal Sheet、真实群消息和 bot 单聊回读已经足够关闭当前 controlled readiness gate；后续产品化仍要扩到更长时间运行样本。2026-05-04 新增的 `scripts/check_workspace_productized_ingestion_readiness.py` 是更严格的 production/full-workspace gate：只有非示例 manifest 同时证明 source adapter coverage、workspace surface coverage、scheduler/cursor、rate-limit/backoff、governance、operations 和 24h+ long-run，才允许 `goal_complete=true`。`scripts/check_workspace_ingestion_objective_completion.py` 会把用户 1-6 项目标映射到 artifact checklist，并把最终完成判定绑定到这个 productized gate。同日新增的 `scripts/run_workspace_ingestion_schedule.py` 是 one-shot 调度入口，默认 plan-only；`scripts/sample_workspace_ingestion_schedule.py` 可连续写出脱敏 schedule reports；`scripts/run_workspace_ingestion_long_run_tick.py` 可被 launchd/cron 每次调用一轮，落盘新 report 后重算 long-run patch、merged manifest 和 objective completion audit；`scripts/collect_workspace_ingestion_long_run_evidence.py` 可把多份脱敏 report 汇总成 production manifest patch；`scripts/collect_workspace_source_coverage_evidence.py` 可把已有真实 gate / workspace ingest / schedule result 报告归一化为 source coverage patch；`scripts/collect_workspace_ops_governance_evidence.py` 可把外部 rate-limit、governance 和 operations 证明归一化为三个 manifest patch section；`scripts/merge_workspace_productized_ingestion_evidence.py` 可把多个 patch 合并成 manifest 并立即跑 gate。最新非示例 partial manifest 已让 `source_coverage`、`discovery_and_cursoring`、`governance`、`operations`、`rate_limit_and_backoff` 全部通过：source adapter 覆盖为 `document_feishu=6`、`lark_sheet=2`、`lark_bitable=6`，workspace surface 覆盖为 `document=2`、`sheet=3`、`bitable=2`、`wiki=1`；单监听、权限负例、audit readback、staging alert rule、停写/备份恢复 drill 和 retention 文档读回也已归一到 evidence refs。2026-05-04 13:38 已安装本机 launchd job `com.feishu-ai-challenge.workspace-ingestion-longrun`，每 4 小时跑一轮受控 non-dry-run schedule，当前 `last exit code=0`。当前唯一 blocker 是 `live_long_run.duration_hours_at_least_24`：10 次非 dry-run schedule execute 成功，`successful_runs=10`、`unresolved_failed_runs=0`，但窗口只有 `1.2473h`。这不是 24h+ 长跑完成证据。
