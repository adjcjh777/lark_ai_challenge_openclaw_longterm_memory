...
4. Phase A 已补齐 storage migration + audit table；当前最大的产品化缺口是：真实 OpenClaw Agent runtime 验收、Feishu staging 单监听流程、live Cognee/Ollama embedding 验证、no-overclaim 交付物审查。
...
2. 真实 runtime 验收前先运行 `python3 scripts/check_feishu_listener_singleton.py --planned-listener openclaw-websocket`，确认没有 legacy `memory_engine feishu listen`、repo 内 `copilot-feishu listen` 或直接 `lark-cli event +subscribe` 同时消费同一个 bot。
3. 在真实 OpenClaw Agent runtime 中至少跑通 3 条：
...
4. 每条记录输入、输出、tool、request_id、trace_id、permission_decision、失败回退。
5. 如果真实 runtime 不稳定，写清失败原因、fallback 到 examples/local bridge 的证据，但不能冒称已完成 runtime。
...
- 验收记录能证明同一时间只有一个 Feishu listener；如果 OpenClaw websocket owns the bot，不再同时运行 lark-cli event listener。

## Phase C：Feishu Staging Runbook

状态：已补单监听守卫和 staging runbook；后续真实 OpenClaw runtime 验收仍按 Phase B 补证据。
...
目标：
1. 新增 docs/productization/openclaw-runtime-evidence.md。
2. 真实 runtime 验收前先跑 `python3 scripts/check_feishu_listener_singleton.py --planned-listener openclaw-websocket`。
3. 在真实 OpenClaw Agent runtime 中至少跑通 3 条：历史决策召回、candidate 创建后确认或拒绝、任务前 memory.prefetch。
4. 每条记录 input、output、tool、request_id、trace_id、permission_decision 和失败回退。
5. 如果真实 runtime 不稳定，写清失败原因和 fallback 到 examples/local bridge 的证据，不能冒称 runtime 已完成。
