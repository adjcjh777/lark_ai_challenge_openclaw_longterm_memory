...
5. 需要宣称“全量 workspace 接入”前，先填充并审计 `deploy/workspace-ingestion.production-evidence.example.json` 的非示例 manifest，再跑 `python3 scripts/check_workspace_productized_ingestion_readiness.py --manifest <manifest> --require-productized-ready --json`。
...
- productized workspace ingestion：已有 one-shot schedule runner、production evidence gate 和 example manifests，但没有非示例 manifest 证明 organic source coverage、真实多轮 scheduler/cursor 运行、rate-limit/backoff、governance、operations 和 24h+ long-run，因此不能说全量 workspace ingestion 完成。
