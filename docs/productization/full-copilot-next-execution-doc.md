...
- 真实飞书权限负例 gate：2026-05-01 新增 `scripts/check_feishu_permission_negative_gate.py --json`，用于检查第二个非 reviewer 真实用户 `@Bot /enable_memory` 的 live denial result；它要求看到 `copilot.group_enable_memory` 的 `permission_denied` result，单独的 `feishu_group_policy_denied` audit 不算通过。当前只是可执行 evidence gate；未导入真实第二用户日志前，不能宣称 live 权限负例已完成。
