...
当前 workspace pilot 已有这些证据：Drive root / folder walk、Wiki space walk、显式 `--resource type:token[:title]`、doc/docx/wiki/sheet/bitable 类型路由、source registry / run registry、revision 去重、unchanged skip、同 filter stale 标记、failed fetch 记录、discovery cursor、registry gate、mixed-source 本地佐证 gate、warm-path latency gate、真实 lark-cli fetch latency gate 和项目 normal Sheet 只读证据 gate。最近本地 warm-path gate 读回平均 5.51ms、最大 5.599ms；真实 lark-cli fetch gate 用公开模板类 Sheet 在临时 DB 中跑完整 fetch / source render / candidate route / registry write，用时 12.747s，`source_count=3`、`candidate_count=2`、`failed_count=0`。项目 normal Sheet 只读 gate 只调用 `sheets +info`，不读单元格；当前发现 2 个 Sheet 候选，项目关键词命中的是 sheet-backed Bitable-only，另一个是跨租户 normal Sheet 且非项目关键词，所以 `eligible_project_normal_sheet_count=0`。

还不能说完成的是：生产全量 workspace ingestion、项目/企业 workspace 内的常规 Sheet 样本、真实 chat + doc/table mixed-source live sample、生产 DB / 生产监控 / productized live 长期运行、完整多租户企业后台。常规 Sheet adapter 和真实 fetch latency gate 都已用公开模板类 Sheet 证明技术路径；项目 normal Sheet gate 证明当前账号还没有合格项目样本，而不是证明企业 workspace 样本已覆盖。
