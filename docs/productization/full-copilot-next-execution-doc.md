...
当前 workspace pilot 已有这些证据：Drive root / folder walk、Wiki space walk、显式 `--resource type:token[:title]`、doc/docx/wiki/sheet/bitable 类型路由、source registry / run registry、revision 去重、unchanged skip、同 filter stale 标记、failed fetch 记录、discovery cursor、registry gate、mixed-source 本地佐证 gate、warm-path latency gate、真实 lark-cli fetch latency gate、真实 docx + Bitable 多来源 temp-DB gate、真实 chat log + workspace resource gate、真实同结论 gate、支持 Drive/Wiki 只读扩搜的真实样本匹配发现器、总 readiness gate 和项目 normal Sheet 只读证据 gate。最近本地 warm-path gate 读回平均 5.51ms、最大 5.599ms；retrieval fallback 已在单次请求内复用 active-memory index 和 curated vector scores，trace 里记录 structured / keyword / vector / Cognee / rerank 的 stage-level `elapsed_ms`；lark-cli wrapper 对成功、解析失败、非零返回、timeout、CLI 缺失和异常路径都记录 `elapsed_ms`；workspace source metadata 也记录 document fetch、Sheet info/read、Bitable record-get timing，方便把本地热路径、子进程边界和飞书网络耗时分开看。真实 lark-cli fetch gate 用公开模板类 Sheet 在临时 DB 中跑完整 fetch / source render / candidate route / registry write，用时 12.747s，`source_count=3`、`candidate_count=2`、`failed_count=0`；真实 docx + Bitable 多来源 gate 用项目提交稿和任务看板在临时 DB 中跑出 1 个 `document_feishu` source、3 个 `lark_bitable` sources、10 个 candidates、0 failed，用时 8.356s；真实 chat log + workspace resource gate 从非 @ 群消息日志提取 1 个 `feishu_message` candidate，并和真实 docx/Base 在同一临时 DB 中跑出 `feishu_message=1`、`document_feishu=1`、`lark_bitable=3`、workspace resource candidates 10、0 failed，用时 7.779s；真实同结论 gate 对同一日志和项目 docx/Base 拉到 4 个 workspace sources，但因 `same_fact_found_in_workspace_source=0` 正确失败；样本发现器现在可直接接收 `--query`、Drive root/folder walk 和 Wiki space walk，最新扩搜扫过 2 份真实日志、4 条 chat message、3 条候选事实、显式项目 docx/Base、Drive root 和 Wiki `my_library`，拉到 48 个 workspace sources（22 `document_feishu`、25 `lark_bitable`、1 `lark_sheet`），仍返回 `same_fact_match_count=0`，并记录 optional_resource_fetch_failure_count=2；显式审阅资源 fetch 失败数为 0；总 readiness gate 当前 `goal_complete=false`，静态架构项全 pass，但项目/企业 normal Sheet 证据和真实同结论佐证仍 fail。项目 normal Sheet 只读 gate 只调用 `sheets +info`，不读单元格；365 天窗口和空查询仍只发现 2 个 Sheet 候选，项目关键词命中的是 sheet-backed Bitable-only，另一个是跨租户 normal Sheet 且非项目关键词，所以 `eligible_project_normal_sheet_count=0`。这些 timing 是 limited pilot 的诊断和防回退证据，不是生产 SLO。

还不能说完成的是：生产全量 workspace ingestion、项目/企业 workspace 内的常规 Sheet 样本、真实 chat + doc/table 围绕同一结论的 corroboration sample、生产 DB / 生产监控 / productized live 长期运行、完整多租户企业后台。常规 Sheet adapter 和真实 fetch latency gate 都已用公开模板类 Sheet 证明技术路径；项目 normal Sheet gate 证明当前账号还没有合格项目样本，而不是证明企业 workspace 样本已覆盖；真实 chat/docx/Bitable gate 证明真实多来源候选共摄入；真实同结论 gate 当前失败，证明现有样本不足以声称同结论佐证闭环。
...
具体采证步骤看 `docs/productization/workspace-ingestion-evidence-collection-runbook.md`。它把 normal Sheet 样本、同结论样本、strict gate、readiness gate、文档更新和看板同步拆成可执行命令，避免把“缺样本”误写成完成。总 readiness gate 现在会复用 `--resource sheet:<token>:<title>` 做项目 normal Sheet evidence，也支持 `--sheet-folder-walk-*` / `--sheet-wiki-space-walk-ids` 直接验收文件夹或知识库空间里的 Sheet；同结论资源池也可通过 `--corroboration-query`、`--corroboration-folder-walk-*` 和 `--corroboration-wiki-space-walk-ids` 做受控只读扩搜，并把显式资源失败和可选 discovery 失败分开计数；只有 Sheet evidence 和同结论 evidence 都通过时，才能把 workspace 目标标记完成。
