...
- `scripts/check_workspace_real_same_conclusion_sample_finder.py`
  - redacted sample finder for existing evidence pools. It scans captured real chat logs and reviewed workspace sources for exact durable-fact matches, and only runs the strict same-conclusion gate when a match exists.
