...
- `memory_engine/feishu_workspace_fetcher.py`
  ...
  - handles the current `drive +search` result shape: `result_meta.doc_types/token/url`, highlighted titles, and Wiki `icon_info.token` for the underlying object token;
  ...
  - inspects Sheet shape with `sheets +info` without reading cells, so project normal Sheet evidence can be checked separately from content ingestion;
...
- `scripts/check_workspace_real_same_conclusion_gate.py`
  - stricter temp-DB gate for captured real chat plus reviewed workspace resources. It first verifies the same durable fact appears in both the chat text and fetched workspace source text, then requires workspace evidence to be added as a duplicate/corroborating source on the active memory version.
...
2026-05-04 controlled evidence: Drive root dry-run returned 8 supported resources; Wiki `my_library` dry-run returned 8 supported resources; combined dry-run returned 16 docx/sheet/bitable resources. Sheet-only direct discovery now respects `--doc-types`: Drive root returned 0 sheets, Wiki `my_library` returned 1 sheet-backed Bitable tab. Temporary SQLite smoke results: Drive folder/root walk ingested 1 docx resource into 1 `document_feishu` source and 2 candidates; Wiki walk with limit 3 fetched 3 resources, produced 2 document sources and 2 candidates, and surfaced the sheet-backed Bitable tab as `stage=no_sources` instead of treating it as a successful source. Repeating the same folder walk in the same temporary SQLite database, then running `check_feishu_workspace_registry_gate.py --min-runs 2 --require-ingested --require-skipped --require-cursor --json`, returned `ok=true`, `run_count=2`, `totals.ingested=1`, `totals.skipped_unchanged=1`, and `cursor_count=1`. A second temporary SQLite run with the same stable folder discovery filter and `--mark-missing-stale` returned `ok=true`, `run_count=2`, `totals.ingested=2`, `totals.skipped_unchanged=1`, `totals.stale_marked=3`, `cursor_count=1`, and `evidence.has_stale=true`. A failed-fetch negative smoke using an invalid explicit docx token returned latest run `status=completed_with_errors`, `totals.failed=1`, `evidence.has_failed=true`, and `status_counts.error=1`. The mixed-source gate returned `ok=true`, `active_evidence_source_types=["document_feishu","feishu_message"]`, `conflict_evidence_source_types=["lark_bitable"]`, and version status counts `active=1`, `candidate=1`. The real docx + Bitable temp-DB gate returned `ok=true`, `source_type_counts.document_feishu=1`, `source_type_counts.lark_bitable=3`, `candidate_count=10`, `failed_count=0`, and `elapsed_ms=8356.391`. The real chat + workspace resource gate returned `ok=true`, `source_type_counts.feishu_message=1`, `source_type_counts.document_feishu=1`, `source_type_counts.lark_bitable=3`, `summary.chat_candidate_count=1`, `summary.resource_candidate_count=10`, `summary.resource_failed_count=0`, and `summary.elapsed_ms=7779.023`. The real same-conclusion gate then fetched the same 4 workspace sources but returned `status=fail` with `same_fact_found_in_workspace_source=0`, proving the current sample is co-ingestion only and not same-conclusion corroboration. The sample finder first scanned 2 real logs, 4 chat messages, 3 chat candidate facts, and 4 workspace sources; it returned `same_fact_match_count=0`. After adding read-only discovery expansion, the finder scanned the same logs plus explicit project docx/Base, Drive root, and Wiki `my_library`, fetched 48 workspace sources (`document_feishu=22`, `lark_bitable=25`, `lark_sheet=1`), and still returned `same_fact_match_count=0`. The finder now reports optional discovery fetch failures separately from explicit reviewed resource failures; in the broad scan, explicit resource fetch failures were 0 and optional discovery fetch failures were 2. The project Sheet evidence gate returned `status=fail` with `candidate_count=2` and `eligible_project_normal_sheet_count=0`: the project keyword match is sheet-backed Bitable-only, while the normal Sheet candidate is cross-tenant and non-project; 365 天窗口和空查询没有改变这个结论. The combined readiness gate returned `goal_complete=false`: static architecture and documentation checks passed, but `project_enterprise_normal_sheet_evidence` and `real_same_conclusion_corroboration_evidence` failed. This proves deterministic folder/wiki discovery, small candidate ingestion, repeat-run skip, same-filter stale marking, failed-fetch registry evidence, local mixed-source corroboration/conflict behavior, current `drive +search` parsing, real docx/Bitable candidate co-ingestion, real chat/docx/Bitable candidate co-ingestion, and a broader read-only sample search work in the current account context; it still does not prove production full-workspace crawling, long-running scheduling, complete enterprise coverage, real project normal-sheet ingestion, or real chat + doc/table same-conclusion corroboration.
