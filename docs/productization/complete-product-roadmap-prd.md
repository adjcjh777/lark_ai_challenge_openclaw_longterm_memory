...
6. 后续若继续推进，`memory.*` first-class OpenClaw 原生工具注册、OpenClaw Feishu websocket running 本机 staging 证据、真实权限映射和 limited Feishu ingestion 本地底座已补；优先补 Feishu Agent tool routing、真实 API 拉取扩样和 productized live，但当前不宣称真实 Feishu DM 已稳定进入本项目 first-class `memory.*` 工具或 production live 已完成。
...
- Phase A 已完成本地 SQLite 兼容迁移：`raw_events`、`memories`、`memory_versions`、`memory_evidence` 已有 `tenant_id`、`organization_id`、`visibility_policy`，并新增 `memory_audit_events`。
...
- 2026-04-28 补充：真实 OpenClaw Agent runtime 受控证据已完成，run `b252f11e-b49d-495c-a14f-0b823a888a5e` 通过 `exec` 调用仓库证据脚本并跑通三条 Copilot flow；Feishu staging runbook 已完成单监听守卫；Phase D live embedding gate 已完成，真实返回 1024 维并清理本项目 Ollama 模型；后续 first-class OpenClaw 原生工具注册、Feishu websocket staging running 证据、真实权限映射和 limited Feishu ingestion 本地底座已补。仍未完成：productized live、真实 Feishu DM 到本项目 first-class `memory.*` tool routing、真实任务/会议/Bitable API 拉取扩样。
