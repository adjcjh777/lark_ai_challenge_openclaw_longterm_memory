...
- Phase 3 Feishu UI / Review Surface 已完成本地闭环和受控飞书可点击卡片路径：card、Bitable dry-run 和 card action 消费 service/tool output，permission denied 不展示未授权 evidence/current_value；Feishu live interactive 候选审核卡可点击确认、拒绝、要求补证据和标记过期，但这不是生产级 card action 长期运行。
...
**目标**：Feishu card/Bitable/review UI 展示 permission-aware candidate/memory，并通过 service 执行 confirm/reject/needs_evidence/expire。
