...
2026-05-04 新增的重点是 workspace ingestion。当前采用 lark-cli-first pilot，已覆盖 Drive / Wiki / Docs / Sheets / Base/Bitable 的受控发现和类型路由，并有 registry skip / stale / failed / cursor、mixed-source 本地佐证、warm-path latency gate 和真实 lark-cli fetch latency gate。公开模板类 normal Sheet 已在临时库读出 `lark_sheet` source 和 candidate；完整 lark-cli fetch / source render / candidate route / registry write 用时 12.747s。它仍缺项目/企业 workspace 内的常规 Sheet 样本和真实 mixed-source live sample。
...
|---|---|---|---|---|---|
| 补项目/企业 workspace 常规 Sheet 样本 | P1 | 程俊豪 | 待定 | `memory_engine/feishu_workspace_fetcher.py`、`scripts/feishu_workspace_ingest.py`、`docs/productization/workspace-ingestion-goal-completion-audit-2026-05-04.md` | 公开模板类 normal Sheet 已证明 `lark_sheet` fetcher 可读出 source/candidate；下一步要用项目/企业 workspace 内已有 normal Sheet token / folder / wiki space 跑通真实只读 discovery 和 `lark_sheet` source -> candidate。如果当前账号没有样本，必须先取得用户明确同意再创建受控测试 Sheet。 |
