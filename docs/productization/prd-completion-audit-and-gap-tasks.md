...
| 补 Feishu live evidence run preflight | P1 | 程俊豪 | 2026-05-02 | `scripts/prepare_feishu_live_evidence_run.py`、`scripts/check_feishu_event_subscription_diagnostics.py`、`tests/test_prepare_feishu_live_evidence_run.py`、`tests/test_feishu_event_subscription_diagnostics.py` | 预检命令不发送消息，只检查 planned listener 是否符合单监听规则，并要求 Feishu message schema 包含 group-message readonly scope，再生成四类日志路径、人工测试步骤、packet 命令和 completion audit 命令。当前机器只看到 `openclaw-gateway` 时，`--planned-listener openclaw-websocket` 通过；若 schema 只列出 `im:message.p2p_msg:readonly`，preflight 会 fail closed，先修 Feishu console scope / event subscription。 |
