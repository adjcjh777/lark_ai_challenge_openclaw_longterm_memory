...
| 任务 | 优先级 | 负责人 | 截止建议 | 文件/页面位置 | 完成标准 |
|---|---|---|---|---|---|
| 固定评委/用户主路径脚本和体验验收 | P0 | 程俊豪 | 待定 | `docs/judge-10-minute-experience.md`、`docs/demo-runbook.md`、`scripts/check_demo_readiness.py`、`docs/productization/handoffs/feishu-dm-routing-handoff.md` | 在一次受控真实 DM `fmc_memory_search` allow-path 证据基础上，固化可复测脚本、用户提示、失败 fallback 和评委 10 分钟入口；仍保持 candidate-only 和 permission fail-closed，不写成稳定长期路由。 |
