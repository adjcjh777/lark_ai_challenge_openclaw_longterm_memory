...
2026-05-04 新增的重点是 workspace ingestion。当前采用 lark-cli-first pilot，已覆盖 Drive / Wiki / Docs / Sheets / Base/Bitable 的受控发现和类型路由，并有 registry skip / stale / failed / cursor、mixed-source 本地佐证和 warm-path latency gate。公开模板类 normal Sheet 已在临时库读出 `lark_sheet` source 和 candidate；它仍缺项目/企业 workspace 内的常规 Sheet 样本、真实 mixed-source live sample 和真实 lark-cli fetch/network latency。
