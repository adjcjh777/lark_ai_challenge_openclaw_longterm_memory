...
| 继续推进长期 Cognee / embedding 服务 | P1 | 程俊豪 | 待定 | `scripts/check_cognee_curated_sync_gate.py`、`memory_engine/copilot/cognee_adapter.py`、`docs/productization/handoffs/cognee-main-path-handoff.md` | 隔离真实 Cognee curated sync gate 已通过，后续仍需受控持久 store、服务健康检查、provider 轮换/限流、备份恢复和长时间运行证据；不得把隔离 gate 写成长期 embedding 服务。 |
