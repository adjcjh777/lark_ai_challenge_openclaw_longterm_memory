...
| 继续推进长期 Cognee / embedding 服务 | P1 | 程俊豪 | 待定 | `scripts/check_cognee_curated_sync_gate.py`、`scripts/sample_cognee_embedding_health.py`、`scripts/collect_cognee_embedding_long_run_evidence.py`、`memory_engine/copilot/cognee_adapter.py`、`docs/productization/handoffs/cognee-main-path-handoff.md` | 隔离真实 Cognee curated sync gate 已通过；embedding health sampler 可持续追加带时间戳的 provider sample；long-run evidence collector 已能要求 Cognee sync pass、persistent store reopened、reopened search/readback pass、embedding sample 窗口 >=24h、成功 sample >=3 和 ops metadata。后续仍需真实受控持久 store、服务健康检查、provider 轮换/限流、备份恢复和长时间运行证据；不得把隔离 gate、sampler 或 collector 写成长期 embedding 服务。 |
