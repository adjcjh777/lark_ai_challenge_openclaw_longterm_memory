...
| 补九项剩余任务 completion audit gate | P1 | 程俊豪 | 2026-05-02 | `scripts/check_openclaw_feishu_productization_completion.py`、`tests/test_openclaw_feishu_productization_completion.py` | 总审计把九项剩余目标逐项映射到 live gate 或 artifact：非 @ 群消息、single listener、first-class `fmc_*` routing、第二真实用户权限负例、真实 `/review` DM/card、Dashboard auth、clean demo DB、Cognee/embedding long-run 和 no-overclaim。2026-05-02 已支持 `--feishu-event-diagnostics` 和 `--cognee-sampler-status`；当前用 12:08 OpenClaw 非 @ 群消息日志重跑时 item 1 已 pass，但 `goal_complete=false`，剩余 blocker 为 first-class 工具扩样、第二真实用户拒绝、真实 `/review` DM/card E2E 和 Cognee sampler 窗口未完成。 |
