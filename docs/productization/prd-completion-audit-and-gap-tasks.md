...
| 补九项剩余任务 completion audit gate | P1 | 程俊豪 | 2026-05-02 | `scripts/check_openclaw_feishu_productization_completion.py`、`tests/test_openclaw_feishu_productization_completion.py` | 总审计把九项剩余目标逐项映射到 live gate 或 artifact：非 @ 群消息、single listener、first-class `fmc_*` routing、第二真实用户权限负例、真实 `/review` DM/card、Dashboard auth、clean demo DB、Cognee/embedding long-run 和 no-overclaim。2026-05-03 传入 Cognee long-run evidence 后 item 8 pass；当前 `goal_complete=false` 的剩余 blocker 为非 @ 群消息 live 投递、first-class 工具扩样、第二真实用户拒绝和真实 `/review` DM/card E2E。 |
