...
|---|---|---|---|
| 是否完成 MVP 构建？ | 已完成可演示、可本地复现、可评测的 MVP 闭环。 | `python3 scripts/check_demo_readiness.py --json` 通过；5 个 demo replay step 全部 pass；benchmark 六类能力全部有 runner。 | 不是生产部署；不是完整多租户后台；storage/audit migration 仍未完成。 |
...
| `memory.confirm` / `memory.reject` 经过治理层 | 完成 | healthcheck candidate review smoke test：candidate -> active；card action 测试走 `handle_tool_request()`。 | 生产前补 audit table，记录每次确认/拒绝。 |
...
| Feishu card / Bitable review surface | 本地闭环完成 | `tests/test_feishu_interactive_cards.py`、`tests/test_bitable_sync.py`；card/Bitable dry-run 消费 service/tool 输出。 | 接真实 card action 前先补 audit table 和 staging runbook。 |
...
|---|---|---|---|---|---|
| 补 storage migration 和 audit table | P0 | 程俊豪 | 2026-05-09 | `memory_engine/`、`docs/productization/contracts/storage-contract.md`、`docs/productization/contracts/audit-observability-contract.md` | 数据库有 `tenant_id`、`organization_id`、`visibility_policy` 和 audit table；healthcheck 不再报 storage warning；确认/拒绝/权限拒绝都有审计记录。 |
