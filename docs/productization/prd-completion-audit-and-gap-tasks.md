...
| 补真实 Feishu/OpenClaw first-class 路由日志审计 gate | P1 | 程俊豪 | 2026-05-02 | `scripts/check_feishu_dm_routing.py`、`tests/test_feishu_dm_routing.py` | `--event-log` 可解析 NDJSON、Copilot listener wrapper 和 OpenClaw gateway 嵌入 JSON，要求真实日志出现 `fmc_*` bridge tool、request_id、trace_id、permission_decision 和 successful result；available live log 只看到 `fmc_memory_create_candidate`，缺 `fmc_memory_search` / `fmc_memory_prefetch`；OpenClaw card-action log 只看到 `fmc_memory_confirm` bridge 但没有 successful result。这个 gate 阻止把内部 `memory.*` 或失败/deny-path 冒称稳定 first-class live routing。 |
| 补九项剩余任务 completion audit gate | P1 | 程俊豪 | 2026-05-02 | `scripts/check_openclaw_feishu_productization_completion.py`、`tests/test_openclaw_feishu_productization_completion.py` | 总审计把九项剩余目标逐项映射到 live gate 或 artifact：非 @ 群消息、single listener、first-class `fmc_*` routing、第二真实用户权限负例、真实 `/review` DM/card、Dashboard auth、clean demo DB、Cognee/embedding long-run 和 no-overclaim。2026-05-02 已支持 `--feishu-event-diagnostics` 和 `--cognee-sampler-status`，可把当前 blocker 明确为 Feishu group-message scope 缺失或 Cognee sampler 窗口未完成。当前 available logs 重跑预期为 `goal_complete=false`，仍不能 overclaim。 |
...
| 补审核卡片 DM/private 定向投递 | P1 | 程俊豪 | 2026-04-30 | `memory_engine/feishu_publisher.py`、`memory_engine/feishu_events.py`、`scripts/check_feishu_review_delivery_gate.py`、`tests/test_feishu_publisher.py`、`tests/test_feishu_interactive_cards.py`、`tests/test_feishu_review_delivery_gate.py` | 带 `open_ids` / `user_ids` 的 interactive card 不再发群 `chat_id`；publisher 逐个 `--user-id` 发送 DM 卡片；失败 fallback 只发 DM 文本；timeout ambiguity 不回群；dry-run 输出 DM mode 和 targets；card action 必须带 Feishu update token，否则 fail closed 且不改候选状态；本地 gate 覆盖 `/review` private card、confirm update 原卡片和缺 token 不变更；`--event-log` gate 可对真实 listener/OpenClaw 日志做候选卡、私聊 DM、点击更新和缺 token negative 的分项判定。当前 available live log 只证明候选审核卡回复，未证明真实 `/review` DM 和真实点击 update_card E2E；不宣称真实飞书长期投递已完成。 |
...
| 补非 @ 群消息事件投递 gate | P1 | 程俊豪 | 2026-05-01 | `scripts/check_feishu_passive_message_event_gate.py`、`scripts/check_feishu_event_subscription_diagnostics.py`、`tests/test_feishu_passive_message_event_gate.py`、`tests/test_feishu_event_subscription_diagnostics.py`、[passive message gate handoff](handoffs/feishu-passive-message-event-gate-handoff.md) | 可对真实 lark-cli/OpenClaw NDJSON/JSON 事件日志检查普通非 @ 群文本是否到达 listener；能解析 Copilot listener `raw_line` wrapper，并明确区分 `reaction_only_no_passive_message_event`、只 @Bot、目标群不匹配和真正 passive message seen。新增只读 lark-cli event diagnostics，不启动 listener，只检查 `im.message.receive_v1` EventKey、required console event、bot auth、lark-cli event bus 与 planned listener 是否冲突。2026-05-01 latest isolated listener log 重跑结果为 `only_at_mention_group_messages_seen`，当前仍不能宣称非 @ 群消息 live 投递已打通。 |
...
| 跑通非 @ 群消息真实投递证据 | P1 | 程俊豪 | 待定 | `scripts/check_feishu_event_subscription_diagnostics.py`、`scripts/check_feishu_passive_message_event_gate.py`、`docs/manual-testing-guide.md`、`docs/productization/feishu-staging-runbook.md` | 先运行只读 event diagnostics，确认 `im.message.receive_v1` 注册、required console event、bot auth 和单监听 owner；再在已启用群策略的真实测试群发送普通非 @ 文本，导出当前单监听入口捕获日志，gate 返回 `passive_group_message_seen`，并继续读回 candidate/audit；如果仍是 reaction-only 或 schema scope 不含 group message 权限，则先修 Feishu app 事件订阅/权限。 |
| 跑通真实飞书权限负例证据 | P1 | 程俊豪 | 待定 | `scripts/check_feishu_permission_negative_gate.py`、`docs/manual-testing-guide.md`、`docs/productization/feishu-staging-runbook.md` | 第二个非 reviewer 真实用户在受控测试群发送 `@Bot /enable_memory`，导出当前单监听入口 result log，gate 返回 `non_reviewer_enable_memory_denied`；audit-only 不算完成，仍不得冒称生产级 RBAC。2026-05-01 gate 已支持 Copilot listener `raw_line` attempt wrapper；available isolated logs 重跑结果仍是 reviewer/admin allow-path，没有第二真实用户 denial result。 |
| 扩大真实飞书卡片点击实测 | P1 | 程俊豪 | 待定 | `memory_engine/copilot/feishu_live.py`、`memory_engine/feishu_events.py`、`scripts/check_feishu_review_delivery_gate.py`、`docs/manual-testing-guide.md` | 本地 gate 已验证 card action update-token 和缺 token fail-closed；`--event-log` gate 已可明确区分“只看到候选审核卡”和完整 private DM / `update_card` / missing-token negative 证据；后续仍需在受控测试群里触发 `/review` 私聊、真实点击 `确认保存`、`拒绝候选`、`要求补证据`、`标记过期` 并读回审计；失败时保留 fallback，不冒称生产级 card action 长期运行。 |
...
| 继续推进长期 Cognee / embedding 服务 | P1 | 程俊豪 | 待定 | `scripts/check_cognee_curated_sync_gate.py`、`scripts/check_cognee_persistent_readback.py`、`scripts/sample_cognee_embedding_health.py`、`scripts/check_cognee_embedding_sampler_status.py`、`scripts/collect_cognee_embedding_long_run_evidence.py`、`memory_engine/copilot/cognee_adapter.py`、`docs/productization/handoffs/cognee-main-path-handoff.md` | 隔离真实 Cognee curated sync gate 已通过；persistent readback gate 可重开同一 store 并搜索已同步 memory；embedding health sampler 可持续追加带时间戳的 provider sample；sampler status gate 可报告进度但不代表完成；long-run evidence collector 已能要求 Cognee sync pass、persistent store reopened、reopened search/readback pass、embedding sample 窗口 >=24h、成功 sample >=3 和 ops metadata。后续仍需真实受控持久 store、服务健康检查、provider 轮换/限流、备份恢复和长时间运行证据；不得把隔离 gate、readback gate、sampler、sampler status 或 collector 写成长期 embedding 服务。 |
