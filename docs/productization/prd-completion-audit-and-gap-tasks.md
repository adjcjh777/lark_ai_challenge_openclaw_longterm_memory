...
| 补 Cognee 主路径本地闭环 | P1 | 程俊豪 | 2026-05-01 | `memory_engine/copilot/cognee_adapter.py`、`memory_engine/copilot/service.py`、`memory_engine/copilot/retrieval.py`、`scripts/check_cognee_curated_sync_gate.py`、`scripts/check_cognee_persistent_readback.py`、`scripts/sample_cognee_embedding_health.py`、`scripts/collect_cognee_embedding_long_run_evidence.py`、[Cognee 主路径 handoff](handoffs/cognee-main-path-handoff.md) | confirm 后只同步 curated memory fields 和 ledger metadata 到 Cognee add -> cognify；reject 会 withdrawal；未匹配 ledger 的 Cognee result 不进入正式 answer；同步失败时 repository fallback 清楚可见；当前安装 Cognee SDK 的 metadata-optional / async 调用形态已兼容，并新增隔离真实 Cognee store gate。2026-05-01 gate 已修正 `.env.local` provider/model 读取顺序，并用 configured custom LLM + OpenAI-compatible embedding 跑通隔离 store，返回 `cognee_sync.status=pass`、`fallback=null`。2026-05-02 新增 persistent readback gate、embedding health sampler 和 long-run evidence collector，可把真实周期 provider sample、curated-sync 报告、persistent store reopen/readback 规范成 completion audit 输入；这仍不是长期 embedding 服务和生产持久化 Cognee 部署。 |
...
| 补九项剩余任务 completion audit gate | P1 | 程俊豪 | 2026-05-02 | `scripts/check_openclaw_feishu_productization_completion.py`、`tests/test_openclaw_feishu_productization_completion.py` | 总审计把九项剩余目标逐项映射到 live gate 或 artifact：非 @ 群消息、single listener、first-class `fmc_*` routing、第二真实用户权限负例、真实 `/review` DM/card、Dashboard auth、clean demo DB、Cognee/embedding long-run 和 no-overclaim。当前 available logs 重跑预期为 `goal_complete=false`，blockers 仍是非 @ 群消息、first-class 工具扩样、第二真实用户拒绝、真实 `/review` DM/card E2E 和 Cognee/embedding 长期服务证据。 |
...
| 补 Feishu live evidence run preflight | P1 | 程俊豪 | 2026-05-02 | `scripts/prepare_feishu_live_evidence_run.py`、`tests/test_prepare_feishu_live_evidence_run.py` | 预检命令不发送消息，只检查 planned listener 是否符合单监听规则，并生成四类日志路径、人工测试步骤、packet 命令和 completion audit 命令。当前机器只看到 `openclaw-gateway` 时，`--planned-listener openclaw-websocket` 通过，repo 内 lark-cli planned listener 会被视为冲突；这减少下一次真实扩样手工拼错日志和命令的风险。 |
...
| 继续推进长期 Cognee / embedding 服务 | P1 | 程俊豪 | 待定 | `scripts/check_cognee_curated_sync_gate.py`、`scripts/check_cognee_persistent_readback.py`、`scripts/sample_cognee_embedding_health.py`、`scripts/collect_cognee_embedding_long_run_evidence.py`、`memory_engine/copilot/cognee_adapter.py`、`docs/productization/handoffs/cognee-main-path-handoff.md` | 隔离真实 Cognee curated sync gate 已通过；persistent readback gate 可重开同一 store 并搜索已同步 memory；embedding health sampler 可持续追加带时间戳的 provider sample；long-run evidence collector 已能要求 Cognee sync pass、persistent store reopened、reopened search/readback pass、embedding sample 窗口 >=24h、成功 sample >=3 和 ops metadata。后续仍需真实受控持久 store、服务健康检查、provider 轮换/限流、备份恢复和长时间运行证据；不得把隔离 gate、readback gate、sampler 或 collector 写成长期 embedding 服务。 |
