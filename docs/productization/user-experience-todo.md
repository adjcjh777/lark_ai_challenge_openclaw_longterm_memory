...
| UX-06 | 真实用户表达样本评测 | 已完成 | 是 | P1 | 样本集、runner 和 pre-live 质量 gate 已完成；当前误记率、解释覆盖率和旧值泄漏率均达标，但仍保留含糊上下文残余样例，不代表生产真实用户稳定可用 |
...
| 6 | UX-06 | [真实用户表达样本评测](user-experience-todos/ux-06-real-user-expression-benchmark.md) | 已覆盖真实表达样本和 UX 指标；pre-live 质量 gate 已通过，残余样例继续进入优化 backlog。 |
...
- 2026-05-03 新增 `scripts/check_real_feishu_expression_quality_gate.py --json`，把 UX-06 阈值变成 pre-live 本地质量 gate。当前 gate 返回 `status=pass`：Recall@3 0.8750、误记率 0.0000、误提醒率 0.0000、解释覆盖率 1.0000、旧值泄漏率 0.0000。
