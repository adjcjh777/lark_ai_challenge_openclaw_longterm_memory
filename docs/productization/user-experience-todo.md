...
| UX-06 | 真实用户表达样本评测 | 已完成 | 是 | P1 | 样本集、runner 和 pre-live 质量 gate 已完成；当前 25 条脱敏样本全部通过，但仍不代表生产真实用户稳定可用 |
...
| 6 | UX-06 | [真实用户表达样本评测](user-experience-todos/ux-06-real-user-expression-benchmark.md) | 已覆盖真实表达样本和 UX 指标；pre-live 质量 gate 已通过，后续继续扩样进入优化 backlog。 |
...
- 难例保留，不删除或简化样本来制造好看的指标。
- 2026-05-03 新增 `scripts/check_real_feishu_expression_quality_gate.py --json`，把 UX-06 阈值变成 pre-live 本地质量 gate。当前 gate 返回 `status=pass`：Recall@3 1.0000、误记率 0.0000、误提醒率 0.0000、解释覆盖率 1.0000、旧值泄漏率 0.0000。
...
- 已完成：新增真实表达样本集，保留难例，不只保留容易成功的样例。
- 已完成：指标包含 Recall@3、误记率、误提醒率、确认负担、回答可解释性、旧值泄漏率。
- 已完成：样本来源脱敏，不提交真实 chat_id、open_id、token。
- 已完成：pre-live 质量 gate 通过；旧 Jenkins 泄漏样例已通过 CI 工具 stable key 和改口 current_value 归一化修复，不删除样本。
...
- 当前 25 条脱敏样本已全部通过；后续应继续扩样而不是删减难例。
