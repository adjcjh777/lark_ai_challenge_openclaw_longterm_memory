...
| Bot DM can recall workspace memory | `agent_adapters/openclaw/plugin/index.js`, `scripts/check_feishu_dm_routing.py`, `tests/test_feishu_dm_routing.py`, `docs/productization/handoffs/feishu-dm-routing-handoff.md` | The first 2026-05-04 p2p DM reached OpenClaw but fell through to the generic agent because the plugin before-dispatch gate only routed group events. Commit `8623342` routes p2p text events through the same first-class memory router and teaches the live log checker to parse raw OpenClaw gateway text lines. After restart, bot single chat `/recall Workspace ingestion readiness gate Sheet 样本` produced a `memory.search` route result whose bridge tool was `fmc_memory_search`; the bot card's first active result was the controlled Sheet fact, with `lark_sheet` evidence, `request_id`, `trace_id`, and `permission: allow / scope_access_granted`. The same command was automatically sent again at 2026-05-04 11:47, 12:53, and 13:08, and read back the same interactive card: first active result was the reviewed normal Sheet fact, route result `ok=true`, bridge tool `fmc_memory_search`, request/trace present, permission `allow / scope_access_granted`, and card delivery `reply_card`. The 13:08 run raised the first-class routing gate to 4 successful `fmc_memory_search` results. | Complete for controlled bot DM readback of workspace memory. Still not proof of stable long-running DM routing or every Feishu conversation entering the tool chain. |
...
4. **A stricter productized workspace ingestion gate now exists, and it is now blocked only by 24h+ long-run evidence.**
   `scripts/check_workspace_productized_ingestion_readiness.py` validates a redacted evidence manifest for organic source coverage, scheduler/cursor, rate-limit/backoff, governance, operations, and 24h+ long-run evidence. The committed example manifest returns `goal_complete=false`; that remains the correct default. The latest non-example active manifest at `logs/workspace-ingestion-productized-probe/20260504T042943Z/productized-evidence.active.json` passes `source_coverage`, `discovery_and_cursoring`, `governance`, `operations`, `rate_limit_and_backoff`, and `secret_redaction`. It uses existing redacted evidence refs for source adapter/surface coverage, same-conclusion, conflict negative, scheduler/cursor, single listener, permission negative, audit readback, staging alert rules, disabled schedule stop-write, backup/restore drill, and retention-policy readback. `scripts/run_workspace_ingestion_long_run_tick.py` now runs one bounded non-dry-run schedule tick, writes a sanitized report, recomputes `long-run-evidence.active.json`, refreshes the merged manifest, and writes `workspace-objective-completion.active.json`. On this machine, launchd job `com.feishu-ai-challenge.workspace-ingestion-longrun` is loaded with a 4-hour interval and has already exited with code 0 after the objective-output update. The remaining blocker is `live_long_run.duration_hours_at_least_24`: `successful_runs=10`, `unresolved_failed_runs=0`, but `duration_hours=1.2473`.
   `scripts/check_workspace_ingestion_objective_completion.py` now sits above that gate. It maps the user's original workspace objective to concrete artifacts, then refuses `goal_complete=true` unless the productized workspace evidence manifest also passes.
