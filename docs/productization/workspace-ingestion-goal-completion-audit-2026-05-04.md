...
python3 scripts/check_workspace_ingestion_goal_readiness.py --json --event-log <captured_non_at_group_message.ndjson> --event-log <captured_first_class_routing.ndjson> --resource 'docx:<project_docx_token>:<title>' --resource 'bitable:<project_base_token>:<title>' --sheet-wiki-space-walk-ids my_library --corroboration-wiki-space-walk-ids my_library --actor-open-id <reviewer_open_id> --roles reviewer --scope workspace:feishu --max-bitable-records 3
```
