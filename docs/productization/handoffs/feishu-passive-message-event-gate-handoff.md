...
| `tests/test_feishu_passive_message_event_gate.py` | 覆盖 passive pass、reaction-only、只 @Bot、目标群不匹配、NDJSON wrapper 和 Copilot listener `raw_line` wrapper。 |
...
- 如果真实日志仍返回 `reaction_only_no_passive_message_event` 或 `only_at_mention_group_messages_seen`，不能说非 @ 群消息 live 投递已打通。
