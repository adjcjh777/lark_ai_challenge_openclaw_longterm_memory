...
4. 判断做对：`python3 scripts/check_feishu_listener_singleton.py --planned-listener openclaw-websocket` 能说明当前没有 lark-cli listener 冲突；`tests.test_feishu_listener_guard` 和 `tests.test_copilot_feishu_live` 通过。
...
- 新增 `tests/test_feishu_listener_guard.py`：覆盖进程识别、冲突判断、OpenClaw websocket 冲突、generic `openclaw-gateway` warning 不阻断。
...
- 当前机器能看到 `openclaw-gateway` 运行；preflight 提示它无法从进程名判断 Feishu websocket 是否启用，因此如果 OpenClaw 正在 owns the bot，就不要启动 lark-cli listener。
...
- Generic `openclaw-gateway` 进程名无法自动证明 Feishu websocket 是否启用；这一步需要按 OpenClaw 配置和现场事实判断。
