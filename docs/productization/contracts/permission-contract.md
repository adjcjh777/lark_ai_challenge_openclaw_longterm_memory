...
日期：2026-05-07
状态：Phase 1 contract freeze 已完成；Phase 2 权限前置实现已完成（commit `b6b17b4`）；2026-04-28 后期打磨已补首批真实飞书权限映射
...
- 已修复首批：飞书 sender open_id 可按本机 staging 配置映射 tenant / organization；chat / document source_context mismatch 会 fail closed。
- 仍未完成：真实通讯录 / 组织架构 API 自动解析、真实文档 ACL、群聊成员权限和长期权限缓存。
...
- `COPILOT_FEISHU_ACTOR_TENANT_MAP`：把飞书 sender open_id 映射为 tenant。
- `COPILOT_FEISHU_ACTOR_ORGANIZATION_MAP`：把飞书 sender open_id 映射为 organization。
- `COPILOT_FEISHU_TENANT_ID` / `COPILOT_FEISHU_ORGANIZATION_ID`：声明当前 staging 允许的真实 tenant / organization。
- `current_context.chat_id` 与 `permission.source_context.chat_id` 不一致时 fail closed。
- `current_context.metadata.document_id` 或 `source_doc_id` 与 `permission.source_context.document_id` 不一致时 fail closed。

仍未完成：

- 自动从飞书通讯录、群聊成员和文档 ACL 解析权限。
- productized live 的长期权限缓存、后台管理和运维告警。
