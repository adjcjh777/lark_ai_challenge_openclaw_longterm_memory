...
- SQLite `SCHEMA_VERSION` 升为 `5`；新增 `tenant_admin_policies` 用于本地/pre-production tenant policy editor；新增 `feishu_group_policies` 用于本地/pre-production 群 onboarding / enable / disable 策略。这仍是本地/pre-production 存储演进，不等于生产图谱服务、全量 workspace ingestion 或完整多租户权限后台。
