...
日期：2026-05-07
状态：Phase A 已实现本地 SQLite 兼容迁移和 audit table；后期打磨 P1 已补 dry-run / apply 迁移入口、索引检查、上线试点存储方案和 Feishu 群图谱节点本地底座；仍不是生产级多租户后台。
...
- 当前没有图谱节点表，无法把同企业下不同飞书群建模成可发现、可审计的上下文节点。
...
用于记录企业内可发现的上下文节点。首版只落本地 SQLite，服务于 Feishu 群节点发现和后续 Cognee/图谱同步，不直接把 raw message 向量化或自动 active。
...
| `node_type` | text | 是 | 首版支持 `organization`、`feishu_chat`。 |
| `node_key` | text | 是 | 外部稳定 ID；Feishu 群为 chat_id。 |
...
| `metadata_json` | text/json | 是 | entrypoint、scope、chat_type、last_message_id 等最小元数据。 |
...
| `tenant_id` | text | 是 | 租户边界。 |
| `organization_id` | text | 是 | 企业/组织边界。 |
| `source_node_id` | text | 是 | 起点节点，首版为 organization。 |
| `target_node_id` | text | 是 | 终点节点，首版为 feishu_chat。 |
| `edge_type` | text | 是 | 首版为 `contains_feishu_chat`。 |
