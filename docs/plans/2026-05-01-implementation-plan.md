...
| 5 | stale leakage 测试 | `retrieval.py`、`tests/test_copilot_governance.py` | 默认 search/prefetch 不泄漏旧值 | stale/superseded leakage 为 0 |
| 6 | 设计 card payload | `feishu_cards.py` | candidate review card 和 version chain card typed payload | card 单测或 snapshot 通过 |
| 7 | 设计 Bitable dry-run | `bitable_sync.py` | 5 类表字段和 dry-run payload 明确 | bitable 单测验证字段齐全 |
| 8 | 建 conflict benchmark | `copilot_conflict_cases.json` | 至少 5-10 条真实冲突表达可跑 | benchmark 或单测计算 accuracy |
