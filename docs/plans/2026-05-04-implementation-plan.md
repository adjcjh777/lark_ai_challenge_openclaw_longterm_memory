...
1. 按 README 从头走一遍，记录卡住的位置；完成标准是核心命令能跑通，卡点写入 handoff。
2. 改 demo 讲解词，让非工程评委能理解“不是普通搜索，而是有证据和版本的企业记忆”；完成标准是 runbook 每步都有白话讲解。
3. 准备截图需求清单：需要截哪个页面、哪个输出、哪个指标表；完成标准是 runbook 写清截图位置。
4. 遇到问题记录：卡住步骤、命令或文档段落、实际输出；完成标准是 handoff 写清失败命令和 fallback。
5. 检查 OpenClaw examples 与 schema 字段一致；完成标准是 examples 能解析，工具名来自 `memory_tools.schema.json`。
