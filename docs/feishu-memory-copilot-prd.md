...
Feishu Memory Copilot 是一个 OpenClaw-native 的企业办公记忆 Copilot。它以 OpenClaw Agent 为主要用户入口和工具编排层，以飞书群聊、文档、Bitable、lark-cli 和飞书 OpenAPI 作为办公集成层，以 Cognee-backed Multi-Level Memory Core 作为长期记忆核心。
...
核心 memory 系统选型锁定为 Cognee。Cognee 在本项目中承担开源知识/记忆引擎职责：本地数据接入、知识图谱和向量检索、dataset 范围组织、可增量处理的记忆构建流程。Feishu Memory Copilot Core 不把 Cognee 当作托管 API 黑盒；本项目自研企业记忆治理层，负责 `candidate / active / superseded / rejected / stale / archived` 状态机、证据链、版本解释、权限门控、OpenClaw tool contract、飞书卡片/Bitable 审核面和 Benchmark 证明。

Cognee 选型依据来自官方文档和代码事实：Cognee 是开源工具和平台，用 relational / vector / graph 三类存储把 raw data 转为可搜索 memory；v1.0 用户级操作是 `remember / recall / improve / forget`，低层 `add / cognify / search` 仍作为可直接控制的构建块；`cognify` 会把已接入数据转为 chunks、embeddings、summaries、nodes 和 edges，并按 dataset 权限执行。参考：`https://docs.cognee.ai/core-concepts/overview`、`https://docs.cognee.ai/core-concepts/main-operations/legacy-operations/cognify`、`https://github.com/topoteretes/cognee`。
...
Cognee 负责底层知识/记忆引擎能力，包括本地数据接入、知识图谱、向量检索和 dataset 范围组织。
Memory Copilot Core 负责企业记忆状态、分层召回、冲突更新、证据链、权限门控和评测证明。
...
决定查哪一层记忆、是否需要 prefetch、是否触发冲突检测
        |
        v
Cognee-backed Multi-Level Memory Core
L0 Working Context
L1 Hot Memory
L2 Warm Memory
L3 Cold Memory
...
dataset / DataPoints / graph store / vector store / recall
        |
        v
Memory Governance
...
### Cognee Integration Boundary

Cognee is the selected memory system core, but not the product boundary.

Cognee owns:

```text
本地 knowledge / memory engine
raw data -> memory 的底层构建能力
dataset 范围组织
graph + vector search substrate
remember / recall / improve / forget workflow
低层 add / cognify / search building blocks
```

Feishu Memory Copilot owns:

```text
OpenClaw tools schema 和 tool handler
candidate / active / superseded / rejected / stale / archived 状态机
evidence / version / recall_logs / permission gate
L0-L3 query cascade 和 prefetch context pack
Feishu card / Bitable review surface
Benchmark runner、失败归因和白皮书证明
```

MVP implementation should use Cognee through a narrow adapter instead of letting product code call Cognee everywhere. The adapter boundary should hide Cognee API changes and convert Cognee recall/search results into Copilot-owned `MemoryResult` objects with status, version, evidence, layer and permission metadata.
...
| L2 Warm Memory | 内存 | 最近 2-7 天记忆、候选记忆、近期文档摘要、未关闭风险 | Copilot store + Cognee recall/search | p95 <= 500ms MVP | 覆盖近期协作上下文 |
| L3 Cold Memory | 硬盘 | 历史 raw events、旧版本、归档文档、长期证据 | Cognee dataset / graph / vector deep search + rerank | p95 <= 2s 或异步 | 追溯历史和补全证据 |
...
- Cognee-backed Multi-Level Memory Core minimal implementation.
...
| D2 | OpenClaw tool schema + Cognee adapter boundary | `agent_adapters/openclaw/`, tool JSON/spec docs, Cognee adapter interface |
| D3 | Multi-Level Memory data model | L0/L1/L2/L3 fields, migration, Cognee dataset/repository boundary |
| D4 | Cognee-backed hybrid retrieval + curated embedding | adapter recall/search, keyword/vector merge, rerank |
