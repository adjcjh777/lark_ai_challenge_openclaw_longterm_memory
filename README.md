...
这还不是生产全量 workspace ingestion。常规 Sheet adapter 已用公开模板类 Sheet 在临时库里读出 `lark_sheet` source 和 candidate，并新增真实 lark-cli fetch latency gate、项目 normal Sheet 只读证据 gate、真实 docx + Bitable temp-DB 多来源摄入证据、真实 chat log + workspace resources 同库共摄入 gate、更严格的真实同结论佐证 gate、支持 Drive/Wiki 只读扩搜的真实样本匹配发现器，以及总 readiness gate。当前 readiness gate 里，lark-cli-first 架构、记忆判断策略、CopilotService 路由复用、共享治理库、性能 gate 和 Opus 4.6 文档边界都 pass；项目/企业 normal Sheet 证据和真实 chat + doc/table 同结论佐证仍 fail。2026-05-04 追加扩搜：项目 Sheet gate 在 365 天窗口和空查询下仍只看到 2 个 Sheet 候选，`eligible_project_normal_sheet_count=0`；新版同结论发现器扫过现有日志、显式项目 docx/Base、Drive root 和 Wiki `my_library` 后拉到 48 个 workspace sources，但 `same_fact_match_count=0`；总 readiness gate 现在也支持 `--corroboration-query`、`--corroboration-folder-walk-*` 和 `--corroboration-wiki-space-walk-ids`，显式资源 fetch 失败和可选 discovery 失败会分开计数。最新总 gate 带 Drive root + Wiki `my_library` 的 Sheet 和 corroboration 只读扩搜仍返回 `goal_complete=false`：显式 fetch failure=0、可选 discovery failure=1、workspace source=17，但 `eligible_project_normal_sheet_count=0`、`same_fact_match_count=0`。因此项目/企业 workspace 内的常规 Sheet 样本、真实 chat + doc/table 围绕同一结论的 corroboration sample，以及 productized live 长期运行都还没有完成。

如果你要继续做 workspace 方向，先看 `docs/productization/workspace-ingestion-goal-completion-audit-2026-05-04.md`、`docs/productization/workspace-ingestion-architecture-adr.md` 和 `docs/productization/workspace-ingestion-evidence-collection-runbook.md`。如果当前是在向用户或队友要样本，直接用 `docs/productization/workspace-ingestion-evidence-request.md`。如果你要做上线或评委材料，先看 `docs/productization/full-copilot-next-execution-doc.md` 和 `docs/productization/prd-completion-audit-and-gap-tasks.md`。
...
| Workspace ingestion pilot | 已新增 lark-cli-first 的受控 workspace adapter：`drive +search`、Drive folder/root walk、Wiki space walk 和显式 `--resource type:token[:title]` 均可发现 doc/docx/wiki/sheet/bitable 资源，按类型路由到 docs/sheets/base fetcher，再统一进入 `FeishuIngestionSource -> ingest_feishu_source() -> CopilotService` candidate pipeline；当前已补 SQLite source registry / run registry / discovery cursor，支持 revision 去重、unchanged skip、同 discovery filter 的 stale 标记、fetch permission denied / not found 的 registry revocation、`--resume-cursor` 续扫、`--skip-discovery` 跳过 search 但保留 folder/wiki walk；direct walk 已遵守 `--doc-types`；新增 `check_feishu_workspace_registry_gate.py` 只读检查 run / registry / cursor / ingested / skipped / stale / failed evidence；2026-05-04 真实只读 discovery：Drive root 发现 8 个资源，Wiki `my_library` 发现 8 个资源，合并 16 个 docx/sheet/bitable 资源；临时 SQLite 已分别跑通受控 Bitable 1 source -> 1 candidate、Drive folder docx 1 source -> 2 candidates、Wiki docx 2 sources -> 2 candidates，sheet-backed Bitable tab 会显式返回 `no_sources`；sheet-only 只读扩样显示 Drive root 0 个 sheet、Wiki `my_library` 1 个 sheet-backed Bitable tab；`docs +search` 找到公开模板类 normal Sheet，临时 SQLite 显式 ingestion 读出 3 个 `lark_sheet` sources、2 个 candidates，registry gate 读回 `ingested_count=3`、`cursor_count=1`；新增真实 lark-cli fetch latency gate：同一路径在临时 DB 中完整跑过 lark-cli fetch / source render / candidate route / registry write，用时 12.747s，`source_count=3`、`candidate_count=2`、`failed_count=0`；新增真实 docx + Bitable 多来源 temp-DB gate：1 个 `document_feishu` source 和 3 个 `lark_bitable` sources 共生成 10 个 candidates、0 failed，用时 8.356s；新增真实 chat log + workspace resource gate：从真实非 @ 群消息日志提取 1 个 `feishu_message` candidate，并和真实 docx/Base 在同一临时 DB 中跑出 `source_type_counts={feishu_message:1, document_feishu:1, lark_bitable:3}`、workspace resource candidates 10、0 failed，用时 7.779s；新增真实同结论 gate：对同一真实日志和项目 docx/Base 只读拉取 4 个 workspace sources 后，因 `same_fact_found_in_workspace_source=0` 明确失败，不把同库共摄入冒称同结论佐证；真实样本匹配发现器现在可用 `--query`、`--folder-walk-root`、`--folder-walk-tokens` 和 `--wiki-space-walk-ids` 扩大只读扫描面；最新扩搜扫过 2 份真实日志、4 条 chat message、3 条候选事实、显式项目 docx/Base、Drive root 和 Wiki `my_library`，拉到 48 个 workspace sources（22 `document_feishu`、25 `lark_bitable`、1 `lark_sheet`），仍返回 `same_fact_match_count=0`，并记录 optional_resource_fetch_failure_count=2；显式审阅资源 fetch 失败数为 0；新增总 readiness gate：当前静态架构项全 pass，但 `project_enterprise_normal_sheet_evidence` 与 `real_same_conclusion_corroboration_evidence` 仍 fail，`goal_complete=false`；总 gate 现在可复用 `--resource sheet:<token>:<title>` 做 Sheet evidence，也支持 `--sheet-folder-walk-*` / `--sheet-wiki-space-walk-ids` 和 `--corroboration-query` / `--corroboration-folder-walk-*` / `--corroboration-wiki-space-walk-ids` 受控只读扩搜。最新带 Drive root + Wiki `my_library` 的总 gate 读回显式 fetch failure=0、可选 discovery failure=1、workspace source=17，但 `eligible_project_normal_sheet_count=0`、`same_fact_match_count=0`；新增项目 normal Sheet 只读证据 gate：365 天窗口和空查询仍只发现 2 个 Sheet 候选，项目关键词命中的是 sheet-backed Bitable-only，另一个是跨租户 normal Sheet 且非项目关键词，`eligible_project_normal_sheet_count=0`；这证明常规 Sheet adapter 可读、真实 fetch latency 可测、当前搜索入口可解析真实 `drive +search` 返回形状、真实 chat/docx/Bitable 可进入同一 temp-DB candidate pipeline，但仍不是项目/企业 workspace 常规 Sheet 样本、生产 SLO 或真实 chat + doc/table 同结论佐证；同一临时库重复 folder walk 的 registry gate 读回 `run_count=2`、`ingested=1`、`skipped_unchanged=1`、`cursor_count=1`；同一临时库缩小 stable folder filter 后用 `--mark-missing-stale` 读回 `stale_marked=3`、`has_stale=true`；显式无效 docx token 负例读回 latest run `completed_with_errors`、`failed=1`、`has_failed=true`；新增 mixed-source gate 证明群消息确认成 active 后，文档同值会追加 `document_feishu` evidence，Bitable 差异值会成为 conflict candidate 且不覆盖 active；新增本地 latency gate，warmup 后文档 ingestion 平均 5.51ms、最大 5.599ms，质量指标全绿；非 dry-run 需要显式 reviewer/operator actor id；`drive +search` query 已加 30 字符上限预检，长 OR 查询会返回结构化错误而不是 traceback；这只是 limited workspace pilot，不是生产全量 workspace ingestion 或长期 crawler | `memory_engine/feishu_workspace_fetcher.py`、`memory_engine/feishu_workspace_registry.py`、`memory_engine/feishu_bitable_fetcher.py`、`scripts/feishu_workspace_ingest.py`、`scripts/check_feishu_workspace_registry_gate.py`、`scripts/check_workspace_mixed_source_corroboration_gate.py`、`scripts/check_workspace_ingestion_latency_gate.py`、`scripts/check_workspace_real_fetch_latency_gate.py`、`scripts/check_workspace_project_sheet_evidence_gate.py`、`scripts/check_workspace_real_chat_resource_gate.py`、`scripts/check_workspace_real_same_conclusion_gate.py`、`scripts/check_workspace_real_same_conclusion_sample_finder.py`、`scripts/check_workspace_ingestion_goal_readiness.py`、`tests/test_feishu_fetchers.py`、`tests/test_feishu_workspace_fetcher.py`、`tests/test_feishu_workspace_registry.py`、`tests/test_feishu_workspace_registry_gate.py`、`tests/test_workspace_mixed_source_corroboration_gate.py`、`tests/test_workspace_ingestion_latency_gate.py`、`tests/test_workspace_real_fetch_latency_gate.py`、`tests/test_workspace_project_sheet_evidence_gate.py`、`tests/test_workspace_real_chat_resource_gate.py`、`tests/test_workspace_real_same_conclusion_gate.py`、`tests/test_workspace_real_same_conclusion_sample_finder.py`、`tests/test_workspace_ingestion_goal_readiness.py`、`docs/productization/workspace-ingestion-architecture-adr.md` |
...
| P1 | 扩大 workspace ingestion pilot 实测 | 已完成显式 Bitable、Drive root/folder walk、Wiki `my_library` walk 的受控 discovery / candidate smoke，并用 registry gate 读回重复运行 skip / cursor、同 filter stale 标记和 failed fetch 负例统计；mixed-source gate 已证明 chat + document 佐证和 Bitable conflict 不覆盖 active；真实 docx + Bitable temp-DB gate 已证明多来源真实 workspace 资源能进入同一 candidate pipeline；真实 chat log + workspace resource gate 已证明真实群消息与真实 docx/Base 能进入同一 temp-DB candidate pipeline；本地 latency gate 已给出 warm path 防回退基线；公开模板类 normal Sheet 已证明 `lark_sheet` fetcher 可读出 source/candidate，并新增真实 lark-cli fetch latency gate；项目 normal Sheet 只读证据 gate 已证明当前账号暂无合格项目 normal Sheet 样本；新版同结论样本发现器已把扫描面扩到显式资源、Drive root 和 Wiki `my_library`，48 个 workspace sources 仍无同事实匹配；总 readiness gate 也支持 `--corroboration-*` 只读扩搜，最新带 Drive root + Wiki `my_library` 的总 gate 仍因 normal Sheet 和同事实样本缺失而 blocked；下一步继续补项目/企业 workspace 内常规 Sheet 样本和真实 chat + doc/table 同结论 corroboration sample；保留 evidence、audit、失败 fallback 和 no-overclaim |
