...
这还不是生产全量 workspace crawler 或长期运行服务。当前已用受控普通 Sheet 和一条真实 OpenClaw Feishu 群消息关闭两个最后证据缺口：项目 normal Sheet gate 读回 `eligible_project_normal_sheet_count=1`，真实 same-fact finder 读回 `same_fact_match_count=1`，strict gate 证明 `feishu_message + lark_sheet` 会合并到同一 active evidence，最终 `check_workspace_ingestion_goal_readiness.py` 返回 `goal_complete=true`、`status=pass`、`failures=[]`。随后又补了四次受控真实 bot 单聊回读：`/recall Workspace ingestion readiness gate Sheet 样本` 通过 OpenClaw first-class router 进入 `fmc_memory_search`，2026-05-04 11:47、12:53 和 13:08 的自动复测都读回 interactive 卡片，第一条 active 记忆就是受控 Sheet 里的 reviewed normal Sheet 结论，并带 `lark_sheet` 证据和 `permission: allow / scope_access_granted` 审计；13:08 复测的脱敏 readback 和 first-class routing gate 保存在 `logs/feishu-dm-workspace-tests/20260504T050822Z/`。这个组合证明当前 workspace ingestion 目标在 pre-production / controlled evidence 边界内可验收；不能写成生产部署、全量企业 workspace 长期扫描、生产 SLO 或 productized live 完成。
...
- OpenClaw websocket 下的受控真实 DM 是另一条验收路径；它已证明 `fmc_memory_search` allow-path，并在 2026-05-04 补过多次 workspace Sheet 记忆回读，最新 13:08 复测仍通过。它仍不等于“所有真实飞书对话都已经稳定进入本项目工具链路”。
