...
Workspace ingestion 已完成 **limited workspace pilot readiness**。现在的主路径是 lark-cli-first：发现 Drive / Wiki / Bitable / Sheet 类资源，按类型读取内容，再统一送入 `FeishuIngestionSource -> ingest_feishu_source() -> memory.create_candidate -> CopilotService`。系统会把低风险内容自动确认，把重要、敏感或冲突内容留给 reviewer / owner。

这还不是生产全量 workspace crawler 或长期运行服务。当前已用受控普通 Sheet 和一条真实 OpenClaw Feishu 群消息关闭两个最后证据缺口：项目 normal Sheet gate 读回 `eligible_project_normal_sheet_count=1`，真实 same-fact finder 读回 `same_fact_match_count=1`，strict gate 证明 `feishu_message + lark_sheet` 会合并到同一 active evidence，最终 `check_workspace_ingestion_goal_readiness.py` 返回 `goal_complete=true`、`status=pass`、`failures=[]`。随后又补了两次受控真实 bot 单聊回读：`/recall Workspace ingestion readiness gate Sheet 样本` 通过 OpenClaw first-class router 进入 `fmc_memory_search`，2026-05-04 11:47 的自动复测再次读回 interactive 卡片，第一条 active 记忆就是受控 Sheet 里的 reviewed normal Sheet 结论，并带 `lark_sheet` 证据和 `permission: allow / scope_access_granted` 审计。这个组合证明当前 workspace ingestion 目标在 pre-production / controlled evidence 边界内可验收；不能写成生产部署、全量企业 workspace 长期扫描、生产 SLO 或 productized live 完成。
...
- OpenClaw websocket 下的受控真实 DM 是另一条验收路径；它已证明 `fmc_memory_search` allow-path，并在 2026-05-04 补过一次 workspace Sheet 记忆回读。它仍不等于“所有真实飞书对话都已经稳定进入本项目工具链路”。
...
| 能力 | 当前状态 | 主要证据 |
|---|---|---|
| OpenClaw memory 工具 | 已完成本机 first-class tool registry、Agent 本地 `fmc_*` 工具调用验证、OpenClaw gateway 本地静默候选筛选和群设置/启停入口，以及受控真实 Feishu/OpenClaw live 证据：2026-04-29 真实 DM -> `fmc_memory_search` -> `CopilotService` allow-path E2E 通过；2026-05-02 `logs/feishu-live-evidence-runs/20260502T085247Z/feishu-live-evidence-packet.json` 已覆盖 `fmc_memory_search`、`fmc_memory_create_candidate`、`fmc_memory_prefetch` 三类 first-class routing success；2026-05-04 修复 OpenClaw `before_dispatch` 的 DM 路由后，真实 bot 单聊 `/recall Workspace ingestion readiness gate Sheet 样本` 被 `fmc_memory_search` 接管并回读 workspace Sheet 记忆，11:47 自动复测再次读回相同 workspace Sheet 记忆卡片和 allow 审计。仍不能写成生产长期运行，也不能写成稳定长期路由 | `agent_adapters/openclaw/plugin/`、`agent_adapters/openclaw/memory_tools.schema.json`、`scripts/check_feishu_dm_routing.py`、`scripts/openclaw_feishu_remember_router.py`、`tests/test_openclaw_tool_registry.py`、`tests/test_feishu_dm_routing.py`、`tests/test_openclaw_feishu_remember_router.py`、`docs/productization/handoffs/feishu-dm-routing-handoff.md` |
...
| Workspace source coverage collector | 已新增 source coverage evidence collector，可从已有真实 gate / workspace ingest / schedule result 报告里抽取 `document_feishu`、`lark_doc`、`lark_sheet`、`lark_bitable`、`wiki` 覆盖、same-conclusion 佐证和 conflict negative 证据，并输出 `production_manifest_patch.source_coverage`。它只归一化已存在的脱敏证据，不运行 ingestion、不证明生产全量完成；当前已修正 sanitized report 同时带 `source_type_counts` 和 `results` 时的重复计数问题 | `scripts/collect_workspace_source_coverage_evidence.py`、`tests/test_workspace_source_coverage_evidence.py` |
...
| Workspace ingestion schedule runner | 已新增 one-shot schedule runner，可从 JSON schedule 构造 bounded `feishu_workspace_ingest.py` job；默认 plan-only 无外部副作用，显式 `--execute` 才串行执行。非 dry-run job 必须带 actor，command preview 会遮蔽资源 token、actor/user/chat id，并支持 bounded retry/backoff。`--output` 会写脱敏 evidence report，保留资源类型和 source type 统计但移除 token/URL/source id/cursor。它补齐 scheduler 入口和受控执行证据，但还不是 24h+ productized long-run evidence | `scripts/run_workspace_ingestion_schedule.py`、`deploy/workspace-ingestion.schedule.example.json`、`tests/test_workspace_ingestion_schedule.py` |
...
| Workspace ingestion long-run collector | 已新增 schedule long-run collector，可读取多份脱敏 schedule report，计算成功运行次数、时间窗口、资源类型分布和未解决失败，并生成 production manifest patch。当前 2 次非 dry-run sampler 正确 blocked：`successful_run_count=2`、`window_hours=0.0129`，不满足 24h+ / 3 runs | `scripts/collect_workspace_ingestion_long_run_evidence.py`、`tests/test_workspace_ingestion_long_run_evidence.py` |
...
| P1 | 扩大 workspace ingestion pilot 实测 | 当前 readiness 已通过：受控普通 Sheet gate 读回 `eligible_project_normal_sheet_count=1`，真实 OpenClaw Feishu 群消息 + Sheet same-fact finder 读回 `same_fact_match_count=1`，strict gate 证明 `feishu_message + lark_sheet` 会合并为同一 active evidence，final readiness gate 返回 `goal_complete=true`、`status=pass`、`failures=[]`。后续扩样转为产品化增强：增加更多真实项目/企业 Sheet、Docx、Base、Wiki space 和长时间运行样本；保留 evidence、audit、失败 fallback 和 no-overclaim |
| P1 | 补 productized workspace ingestion evidence gate | 已新增 `check_workspace_productized_ingestion_readiness.py`，把全量 workspace 接入的完成条件拆成 source coverage、scheduler/cursor、rate-limit/backoff、governance、operations、24h+ long-run 六类证据；当前默认示例 manifest 明确 blocked，不代表生产全量完成 |
| P1 | 补 workspace objective completion audit | 已新增 `check_workspace_ingestion_objective_completion.py`，把用户原始 1-6 项目标映射到 artifact checklist，并要求 productized readiness gate 在非示例 manifest 上通过才允许 `goal_complete=true` |
| P1 | 补 workspace source coverage collector | 已新增 `collect_workspace_source_coverage_evidence.py`，从已有脱敏 evidence reports 归一化 source coverage patch；2026-05-04 非 dry-run sampler 读回 `document_feishu=8`、`lark_bitable=43`，但 `lark_doc`、`lark_sheet`、`wiki`、same-conclusion、conflict-negative 仍 blocked |
| P1 | 补 workspace ops/governance collector | 已新增 `collect_workspace_ops_governance_evidence.py`，把 rate-limit、governance、operations 外部证明整理成 manifest patch；collector 通过不等于生产 ingestion 运行完成 |
| P1 | 补 workspace productized evidence merger | 已新增 `merge_workspace_productized_ingestion_evidence.py`，合并多个 `production_manifest_patch` 并立即跑 readiness gate，拒绝未知 section、placeholder 和 secret-like 值 |
| P1 | 补 workspace ingestion schedule runner | 已新增 `run_workspace_ingestion_schedule.py` 和 example schedule，默认只生成计划；后续可由 cron/launchd/systemd timer 显式执行并产出 long-run evidence |
| P1 | 补 workspace ingestion schedule sampler | 已新增 `sample_workspace_ingestion_schedule.py`，可连续收集脱敏 schedule reports 和 sampler status；默认 plan-only，显式 `--execute` 才采集 bounded dry-run / live schedule evidence |
