...
| 非 @ 群消息事件 gate | 已新增脱敏事件日志检查脚本，可区分普通非 @ 群文本已到达、只看到 @Bot 消息、只看到 reaction 事件或目标群不匹配；当前仍需用真实 Feishu 捕获日志跑通后才可宣称 live passive 投递完成 | `scripts/check_feishu_passive_message_event_gate.py`、`tests/test_feishu_passive_message_event_gate.py`、`docs/productization/handoffs/feishu-passive-message-event-gate-handoff.md` |
