...
这还不是生产全量 workspace ingestion。常规 Sheet adapter 已用公开模板类 Sheet 在临时库里读出 `lark_sheet` source 和 candidate，并新增真实 lark-cli fetch latency gate；但项目/企业 workspace 内的常规 Sheet 样本、真实 chat + doc/table mixed-source live sample，以及 productized live 长期运行都还没有完成。
