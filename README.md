...
| Productized workspace ingestion gate | 已新增严格生产化证据 gate：只有非示例 manifest 同时证明 organic source coverage、scheduler/cursor、rate-limit/backoff、governance、operations 和 24h+ long-run，才允许 `goal_complete=true`。默认示例 manifest 会返回 `status=blocked`，防止把 limited pilot 写成全量 workspace ingestion | `scripts/check_workspace_productized_ingestion_readiness.py`、`deploy/workspace-ingestion.production-evidence.example.json`、`tests/test_workspace_productized_ingestion_readiness.py` |
...
| Workspace ingestion schedule runner | 已新增 one-shot schedule runner，可从 JSON schedule 构造 bounded `feishu_workspace_ingest.py` job；默认 plan-only 无外部副作用，显式 `--execute` 才串行执行。非 dry-run job 必须带 actor，command preview 会遮蔽资源 token，并支持 bounded retry/backoff。2026-05-04 用 example schedule 执行了一次 dry-run execute，只做真实 discovery、不 fetch 内容、不写 DB，读回 `resource_count=34`、`status=pass`。`--output` 会写脱敏 evidence report，保留资源类型统计但移除 token/URL。它补齐 scheduler 入口和一次只读执行证据，但还不是 24h+ productized long-run evidence | `scripts/run_workspace_ingestion_schedule.py`、`deploy/workspace-ingestion.schedule.example.json`、`tests/test_workspace_ingestion_schedule.py` |
| Workspace ingestion schedule sampler | 已新增周期采样器，可按 `sample-count` / `interval-seconds` 连续运行 schedule runner，并把每轮脱敏 report、NDJSON index 和 `sampler-status.json` 写到本地证据目录。默认 plan-only；显式 `--execute` 才运行 bounded schedule。它让 24h+ evidence 可以自动采集，但采样窗口未满足前仍不能宣称 productized full-workspace readiness | `scripts/sample_workspace_ingestion_schedule.py`、`tests/test_workspace_ingestion_schedule_sampler.py` |
| Workspace ingestion long-run collector | 已新增 schedule long-run collector，可读取多份脱敏 schedule report，计算成功运行次数、时间窗口、资源类型分布和未解决失败，并生成 production manifest patch。当前单报告采集会正确 blocked：`successful_run_count=1`、`window_hours=0`，不满足 24h+ / 3 runs | `scripts/collect_workspace_ingestion_long_run_evidence.py`、`tests/test_workspace_ingestion_long_run_evidence.py` |
...
| P1 | 补 workspace source coverage collector | 已新增 `collect_workspace_source_coverage_evidence.py`，从已有脱敏 evidence reports 归一化 source coverage patch；source coverage patch 可以进 merger，但不替代真实 organic 样本 |
