...
这还不是生产全量 workspace crawler 或长期运行服务。当前已用受控普通 Sheet 和一条真实 OpenClaw Feishu 群消息关闭两个最后证据缺口：项目 normal Sheet gate 读回 `eligible_project_normal_sheet_count=1`，真实 same-fact finder 读回 `same_fact_match_count=1`，strict gate 证明 `feishu_message + lark_sheet` 会合并到同一 active evidence，最终 `check_workspace_ingestion_goal_readiness.py` 返回 `goal_complete=true`、`status=pass`、`failures=[]`。随后又补了三次受控真实 bot 单聊回读：`/recall Workspace ingestion readiness gate Sheet 样本` 通过 OpenClaw first-class router 进入 `fmc_memory_search`，2026-05-04 11:47 和 12:53 的自动复测都读回 interactive 卡片，第一条 active 记忆就是受控 Sheet 里的 reviewed normal Sheet 结论，并带 `lark_sheet` 证据和 `permission: allow / scope_access_granted` 审计；12:53 复测的脱敏 readback 和 first-class routing gate 保存在 `logs/feishu-dm-workspace-tests/20260504T125310Z/`。这个组合证明当前 workspace ingestion 目标在 pre-production / controlled evidence 边界内可验收；不能写成生产部署、全量企业 workspace 长期扫描、生产 SLO 或 productized live 完成。
...
- OpenClaw websocket 下的受控真实 DM 是另一条验收路径；它已证明 `fmc_memory_search` allow-path，并在 2026-05-04 补过多次 workspace Sheet 记忆回读，最新 12:53 复测仍通过。它仍不等于“所有真实飞书对话都已经稳定进入本项目工具链路”。
...
| 能力 | 当前状态 | 主要证据 |
|---|---|---|
| OpenClaw memory 工具 | 已完成本机 first-class tool registry、Agent 本地 `fmc_*` 工具调用验证、OpenClaw gateway 本地静默候选筛选和群设置/启停入口，以及受控真实 Feishu/OpenClaw live 证据：2026-04-29 真实 DM -> `fmc_memory_search` -> `CopilotService` allow-path E2E 通过；2026-05-02 `logs/feishu-live-evidence-runs/20260502T085247Z/feishu-live-evidence-packet.json` 已覆盖 `fmc_memory_search`、`fmc_memory_create_candidate`、`fmc_memory_prefetch` 三类 first-class routing success；2026-05-04 修复 OpenClaw `before_dispatch` 的 DM 路由后，真实 bot 单聊 `/recall Workspace ingestion readiness gate Sheet 样本` 被 `fmc_memory_search` 接管并回读 workspace Sheet 记忆，11:47 和 12:53 自动复测都读回相同 workspace Sheet 记忆卡片和 allow 审计。仍不能写成生产长期运行，也不能写成稳定长期路由 | `agent_adapters/openclaw/plugin/`、`agent_adapters/openclaw/memory_tools.schema.json`、`scripts/check_feishu_dm_routing.py`、`scripts/openclaw_feishu_remember_router.py`、`tests/test_openclaw_tool_registry.py`、`tests/test_feishu_dm_routing.py`、`tests/test_openclaw_feishu_remember_router.py`、`docs/productization/handoffs/feishu-dm-routing-handoff.md` |
...
| Workspace ingestion long-run collector | 已新增 schedule long-run collector，可读取多份脱敏 schedule report，计算成功运行次数、时间窗口、资源类型分布和未解决失败，并生成 production manifest patch。当前 4 次非 dry-run report 已让 `successful_runs_at_least_3` 通过，`revision_skip_proven=true`、`revocation_proven=true`、`stale_marking_proven=true`、`cursor_resume_proven=true`；仍因 `window_hours=0.191` 未满足 24h+ | `scripts/collect_workspace_ingestion_long_run_evidence.py`、`tests/test_workspace_ingestion_long_run_evidence.py` |
...
| P1 | 补 workspace source coverage collector | 已新增 `collect_workspace_source_coverage_evidence.py`，从已有脱敏 evidence reports 归一化 source coverage patch；2026-05-04 非 dry-run sampler 读回 `document_feishu=8`、`lark_bitable=43`，现在 gate 分别检查 source adapter 与 workspace surface，仍缺满足非示例 manifest 的完整 source/surface 覆盖、same-conclusion 和 conflict-negative |
