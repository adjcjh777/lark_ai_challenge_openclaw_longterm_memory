...
| Workspace ingestion pilot | 已新增 lark-cli-first 的受控 workspace adapter：`drive +search` 做 doc/docx/wiki/sheet/bitable 资源发现，按类型路由到 docs/sheets/base fetcher，再统一进入 `FeishuIngestionSource -> ingest_feishu_source() -> CopilotService` candidate pipeline；当前已补 SQLite source registry / run registry / discovery cursor，支持 revision 去重、unchanged skip、同 discovery filter 的 stale 标记、fetch permission denied / not found 的 registry revocation、`--resume-cursor` 续扫，以及 `--resource type:token[:title] --skip-discovery` 的显式资源入口；2026-05-04 已用临时 SQLite 对受控 Bitable token 跑通 1 个 `lark_bitable` source -> 1 个 candidate；非 dry-run 需要显式 reviewer/operator actor id；这只是 limited workspace pilot，不是生产全量 workspace ingestion 或长期 crawler | `memory_engine/feishu_workspace_fetcher.py`、`memory_engine/feishu_workspace_registry.py`、`memory_engine/feishu_bitable_fetcher.py`、`scripts/feishu_workspace_ingest.py`、`tests/test_feishu_fetchers.py`、`tests/test_feishu_workspace_fetcher.py`、`tests/test_feishu_workspace_registry.py`、`docs/productization/workspace-ingestion-architecture-adr.md` |
...
| P1 | 扩大 workspace ingestion pilot 实测 | 已完成 `--resource bitable:... --skip-discovery` 的受控 Bitable candidate smoke；下一步继续用 `scripts/feishu_workspace_ingest.py --dry-run --mine --opened-since 30d --sort edit_time --json` 或受控 folder/wiki space 读回 Drive 发现结果，再用 reviewer/operator actor id 小批量跑 doc/sheet/bitable candidate-only ingestion；第二次运行必须读回 registry skip / stale / failed / cursor 统计，保留 evidence、audit、失败 fallback 和 no-overclaim |
