...
| 能力 | 当前状态 | 主要证据 |
|---|---|---|
| OpenClaw memory 工具 | 已完成本机 first-class tool registry、本地 `fmc_*` 工具调用、gateway 静默筛选和受控 Feishu live 证据；仍不能写成生产长期运行或稳定长期路由 | `agent_adapters/openclaw/plugin/`、`agent_adapters/openclaw/memory_tools.schema.json`、`scripts/check_feishu_dm_routing.py`、`scripts/openclaw_feishu_remember_router.py`、`docs/productization/handoffs/feishu-dm-routing-handoff.md` |
| Copilot core / 权限 / 治理 | 已完成服务入口、fail-closed 权限门控、candidate / confirm / reject / conflict / version chain、分层检索和 SQLite 审计闭环 | `memory_engine/copilot/service.py`、`tools.py`、`permissions.py`、`governance.py`、`retrieval.py`、`memory_engine/db.py`、`tests/test_copilot_*` |
| 飞书审核体验 | 已完成受控 sandbox/pre-production 路径：interactive card、定向 reviewer/owner 收件箱、DM 投递、撤销、补证据和冲突合并；仍不能写成生产级长期投递 | `memory_engine/copilot/feishu_live.py`、`review_inbox.py`、`memory_engine/feishu_cards.py`、`memory_engine/feishu_events.py`、`memory_engine/feishu_publisher.py`、`docs/productization/handoffs/real-feishu-*-handoff.md` |
| 存储、备份和图谱 | 已完成本地 migration dry-run / apply、SQLite staging backup / restore drill，以及 Feishu 群/用户/消息拓扑建模；这不是生产 PostgreSQL / PITR | `scripts/migrate_copilot_storage.py`、`scripts/backup_copilot_storage.py`、`memory_engine/copilot/graph_context.py`、`docs/productization/handoffs/feishu-group-graph-node-handoff.md` |
...
| Cognee 主路径 | 已完成本地/staging 的 curated memory 同步、检索、fallback、持久 store readback 和 embedding health 证据；这不是生产持久化 Cognee 服务 | `memory_engine/copilot/cognee_adapter.py`、`memory_engine/copilot/retrieval.py`、`scripts/check_cognee_*`、`scripts/collect_cognee_embedding_long_run_evidence.py`、`docs/productization/handoffs/cognee-main-path-handoff.md` |
| Feishu live / ingestion | 已完成受控测试群、群级设置启停、静默 candidate probe、文档/任务/会议/Bitable fetcher 和 review policy；这不是被动全量群聊摄入 | `memory_engine/copilot/feishu_live.py`、`group_policies.py`、`memory_engine/document_ingestion.py`、`memory_engine/feishu_*_fetcher.py`、`tests/test_copilot_feishu_live.py`、`tests/test_feishu_fetchers.py` |
| Workspace ingestion pilot | 已完成 lark-cli-first 的受控 discovery / fetch / candidate pipeline，覆盖 Drive、Wiki、Docx、Sheet、Bitable、registry、cursor、skip/stale/failure 和同结论回读；这是 limited pilot，不是生产全量 workspace ingestion | `memory_engine/feishu_workspace_fetcher.py`、`memory_engine/feishu_workspace_registry.py`、`scripts/feishu_workspace_ingest.py`、`scripts/check_workspace_*`、`docs/productization/workspace-ingestion-goal-completion-audit-2026-05-04.md` |
| Productized workspace ingestion gate | 已有严格生产化 gate：非示例 manifest 必须同时证明 source/surface coverage、scheduler/cursor、rate-limit/backoff、governance、operations 和 24h+ long-run；当前仍未通过 24h+ 长跑 | `scripts/check_workspace_productized_ingestion_readiness.py`、`deploy/workspace-ingestion.production-evidence.example.json`、`tests/test_workspace_productized_ingestion_readiness.py` |
| Workspace productized evidence 工具链 | 已完成 objective audit、source/ops/governance/long-run collectors、patch merger、schedule runner/sampler 和只读 finalizer；这些脚本只整理受控证据，不替代真实生产证据，当前仍卡在 24h+ long-run | `scripts/check_workspace_ingestion_objective_completion.py`、`scripts/collect_workspace_*_evidence.py`、`scripts/run_workspace_ingestion_*`、`scripts/merge_workspace_productized_ingestion_evidence.py`、`scripts/finalize_workspace_ingestion_productized_evidence.py` |
