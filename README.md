...
| 九项剩余任务 completion audit | 已新增总审计 gate，把非 @ live 投递、单监听、first-class routing、权限负例、review DM/card、Dashboard auth、clean demo DB、Cognee/embedding 长期服务和 no-overclaim 映射到具体证据；可选读取 Feishu event diagnostics 和 Cognee sampler status 来给出更精确 blocker；当前包含这些输入后仍返回 `goal_complete=false`，blockers 为 Feishu group-message scope、first-class 工具扩样、第二真实用户拒绝、真实 `/review` DM/card E2E 和 Cognee sampler 窗口未完成 | `scripts/check_openclaw_feishu_productization_completion.py`、`tests/test_openclaw_feishu_productization_completion.py` |
...
| Feishu live evidence run preflight | 已新增非发送型 live run 预检：检查当前单监听 owner 和 group-message event scope，生成四类日志路径、人工测试步骤、packet 命令和 completion audit 命令；当前机器只看到 `openclaw-gateway`，因此计划 listener 应保持 `openclaw-websocket`，不要再启动 lark-cli listener 抢同一个 bot；若 schema 只列出 `im:message.p2p_msg:readonly`，preflight 会 fail closed | `scripts/prepare_feishu_live_evidence_run.py`、`scripts/check_feishu_event_subscription_diagnostics.py`、`tests/test_prepare_feishu_live_evidence_run.py`、`tests/test_feishu_event_subscription_diagnostics.py` |
...
`check_openclaw_feishu_productization_completion.py` 会把本轮九项剩余任务映射到真实 gate 和 artifact：默认读取当前 available Feishu/OpenClaw logs，要求非 @ 群文本、first-class `fmc_*` 成功结果、第二真实用户 deny、真实 `/review` DM/update-card，以及 Cognee/embedding 持久 long-run 证据。真实复核时建议把 `check_feishu_event_subscription_diagnostics.py --require-group-message-scope` 的 JSON 通过 `--feishu-event-diagnostics` 输入，把 `check_cognee_embedding_sampler_status.py` 的 JSON 通过 `--cognee-sampler-status` 输入，这样 audit 会明确区分 Feishu scope blocker、sampler still-running 和最终 evidence missing。当前预期输出是 `goal_complete=false`，不能用它的局部 pass 项 overclaim 整体完成。
...
`prepare_feishu_live_evidence_run.py` 会在真实扩样前做单监听和 Feishu group-message scope preflight，并生成一次 run 的日志路径、人工消息步骤、packet 命令和 completion audit 命令。它不会发送飞书消息或点击卡片；如果只看到泛化 `openclaw-gateway`，只有 `--planned-listener openclaw-websocket` 会通过，repo 内 lark-cli listener 应继续 fail closed；如果诊断只看到 `im:message.p2p_msg:readonly` 而没有 group message readonly scope，则 live passive 投递扩样应先停下修 Feishu console 配置。
