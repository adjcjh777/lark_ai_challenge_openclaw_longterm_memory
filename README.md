...
| 能力 | 当前状态 | 主要证据 |
|---|---|---|
| OpenClaw memory 工具 | 已完成本机 first-class tool registry、Agent 本地 `fmc_*` 工具调用验证、OpenClaw gateway 本地静默候选筛选和群设置/启停入口，以及一次受控真实 Feishu DM -> `fmc_memory_search` -> `CopilotService` allow-path live E2E 证据；`scripts/check_feishu_dm_routing.py --event-log` 可审计真实日志里的 `fmc_*` bridge 结果，当前 available logs 仍缺 `fmc_memory_search` / `fmc_memory_prefetch` 扩样成功证据，不能写成稳定长期路由 | `agent_adapters/openclaw/plugin/`、`agent_adapters/openclaw/memory_tools.schema.json`、`scripts/check_feishu_dm_routing.py`、`scripts/openclaw_feishu_remember_router.py`、`tests/test_openclaw_tool_registry.py`、`tests/test_feishu_dm_routing.py`、`tests/test_openclaw_feishu_remember_router.py`、`docs/productization/handoffs/feishu-dm-routing-handoff.md` |
...
| 定向审核收件箱、DM 投递与撤销/合并体验 | 已完成本地受控路径：`/review` 默认打开“待我审核”收件箱卡片，只对当前 reviewer/owner 定向可见；定向 interactive card 由 publisher 逐个 `open_id/user_id` 发送 DM，不再回群；收件箱可按 mine/conflicts/high_risk 查看并直接点击确认、拒绝、补证据；冲突候选卡显示旧结论/新结论并提供“确认合并”；`/undo` 和 card action router 可把已确认/拒绝/补证据/过期的候选撤回待审核；`scripts/check_feishu_review_delivery_gate.py` 本地验证 private review card、card-action update 和缺 token fail-closed，`--event-log` 可审计真实 listener/OpenClaw 日志；2026-05-01 available live log 只证明候选审核卡回复，尚未证明真实 `/review` 私聊 DM 和真实点击 update_card E2E | `memory_engine/copilot/review_inbox.py`、`memory_engine/copilot/feishu_live.py`、`memory_engine/feishu_cards.py`、`memory_engine/feishu_events.py`、`memory_engine/feishu_publisher.py`、`scripts/openclaw_feishu_card_action_router.py`、`scripts/check_feishu_review_delivery_gate.py`、`tests/test_copilot_review_inbox.py`、`tests/test_copilot_feishu_live.py`、`tests/test_feishu_publisher.py`、`tests/test_openclaw_feishu_card_action_router.py`、`tests/test_feishu_review_delivery_gate.py` |
...
| Cognee 主路径 | 已完成本地可控同步 / 检索 / fallback 闭环；当前 SDK metadata-optional / async 形态已兼容；真实 Cognee curated sync gate 已按 `.env.local` provider 配置跑通隔离 store 的 `CopilotService.confirm -> Cognee add -> cognify`，`fallback=null`；已新增 Cognee/embedding long-run evidence collector 和 embedding health sampler，可把 curated-sync 报告、持久 store reopen/readback 和周期 embedding sample 规范成 completion audit 输入；长期 embedding 服务仍未完成 | `memory_engine/copilot/cognee_adapter.py`、`memory_engine/copilot/retrieval.py`、`scripts/check_cognee_curated_sync_gate.py`、`scripts/sample_cognee_embedding_health.py`、`scripts/collect_cognee_embedding_long_run_evidence.py`、`tests/test_copilot_cognee_adapter.py`、`tests/test_cognee_curated_sync_gate.py`、`tests/test_cognee_embedding_health_sampler.py`、`tests/test_cognee_embedding_long_run_evidence.py`、`docs/productization/handoffs/cognee-main-path-handoff.md` |
...
| 非 @ 群消息事件 gate | 已新增脱敏事件日志检查脚本，可解析 Feishu 原始 payload、NDJSON wrapper 和 Copilot listener `raw_line` wrapper，并区分普通非 @ 群文本已到达、只看到 @Bot 消息、只看到 reaction 事件或目标群不匹配；2026-05-01 latest isolated listener log 重跑结果为 `only_at_mention_group_messages_seen`，当前仍需新的真实非 @ 群文本捕获日志跑通后才可宣称 live passive 投递完成 | `scripts/check_feishu_passive_message_event_gate.py`、`tests/test_feishu_passive_message_event_gate.py`、`docs/productization/handoffs/feishu-passive-message-event-gate-handoff.md` |
| 九项剩余任务 completion audit | 已新增总审计 gate，把非 @ live 投递、单监听、first-class routing、权限负例、review DM/card、Dashboard auth、clean demo DB、Cognee/embedding 长期服务和 no-overclaim 映射到具体证据；当前默认读取 available logs 后返回 `goal_complete=false`，blockers 为非 @ 群消息、first-class 工具扩样、第二真实用户拒绝、真实 `/review` DM/card E2E 和 Cognee/embedding long-run | `scripts/check_openclaw_feishu_productization_completion.py`、`tests/test_openclaw_feishu_productization_completion.py` |
...
| Feishu live evidence run preflight | 已新增非发送型 live run 预检：检查当前单监听 owner，生成四类日志路径、人工测试步骤、packet 命令和 completion audit 命令；当前机器只看到 `openclaw-gateway`，因此计划 listener 应保持 `openclaw-websocket`，不要再启动 lark-cli listener 抢同一个 bot | `scripts/prepare_feishu_live_evidence_run.py`、`tests/test_prepare_feishu_live_evidence_run.py` |
...
2026-05-02 追加：`scripts/sample_cognee_embedding_health.py` 会把 `check_embedding_provider.py` 的真实结果追加成带 `sampled_at` 的 NDJSON，可用于 24 小时采样窗口；`scripts/collect_cognee_embedding_long_run_evidence.py` 会把真实 curated-sync report、持久 store reopen/readback 证明和周期 embedding sample log 规范成 `check_openclaw_feishu_productization_completion.py --cognee-long-run-evidence` 可读取的 JSON。默认完成标准要求 Cognee sync pass、store reopened、reopened search/readback pass、embedding sample 窗口 >=24h、成功 sample >=3，并带 service owner / evidence ref；这些脚本只采集/规范证据，不创建长期服务。
...
`check_openclaw_feishu_productization_completion.py` 会把本轮九项剩余任务映射到真实 gate 和 artifact：默认读取当前 available Feishu/OpenClaw logs，要求非 @ 群文本、first-class `fmc_*` 成功结果、第二真实用户 deny、真实 `/review` DM/update-card，以及 Cognee/embedding 持久 long-run 证据。当前预期输出是 `goal_complete=false`，不能用它的局部 pass 项 overclaim 整体完成。

`collect_feishu_live_evidence_packet.py` 会把四类 Feishu/OpenClaw live log 统一跑 gate，生成不含 raw 消息正文的 sanitized evidence packet。真实扩样时先用它生成 packet，再运行 `check_openclaw_feishu_productization_completion.py --feishu-live-evidence-packet <packet> --cognee-long-run-evidence <json>`；packet 只是证据汇总，不能替代真实日志采集。

`prepare_feishu_live_evidence_run.py` 会在真实扩样前做单监听 preflight 并生成一次 run 的日志路径、人工消息步骤、packet 命令和 completion audit 命令。它不会发送飞书消息或点击卡片；如果只看到泛化 `openclaw-gateway`，只有 `--planned-listener openclaw-websocket` 会通过，repo 内 lark-cli listener 应继续 fail closed。
