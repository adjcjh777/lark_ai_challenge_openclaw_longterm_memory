...
| Workspace ingestion schedule runner | 已新增 one-shot schedule runner，可从 JSON schedule 构造 bounded `feishu_workspace_ingest.py` job；默认 plan-only 无外部副作用，显式 `--execute` 才串行执行。非 dry-run job 必须带 actor，command preview 会遮蔽资源 token，并支持 bounded retry/backoff。它补齐 scheduler 入口，但还不是 24h+ productized long-run evidence | `scripts/run_workspace_ingestion_schedule.py`、`deploy/workspace-ingestion.schedule.example.json`、`tests/test_workspace_ingestion_schedule.py` |
