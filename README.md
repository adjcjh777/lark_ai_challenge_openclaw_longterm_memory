...
| 非 @ 群消息事件 gate | 已新增脱敏事件日志检查脚本，可解析 Feishu 原始 payload、NDJSON wrapper、Copilot listener `raw_line` wrapper 和 OpenClaw channel log；历史 2026-05-02 12:08 受控日志曾证明一条普通非 @ 文本进入 OpenClaw websocket，但 2026-05-03 read-only preflight 显示当前 Feishu message event schema 仍只列出 `im:message.p2p_msg:readonly`，缺 bot 侧 `im:message.group_msg` / `im:message.group_msg:readonly`，因此新的非 @ 群消息采证必须先修 Feishu console scope/event subscription 后再跑 | `scripts/check_feishu_passive_message_event_gate.py`、`scripts/check_feishu_event_subscription_diagnostics.py`、`tests/test_feishu_passive_message_event_gate.py`、`tests/test_feishu_event_subscription_diagnostics.py`、`docs/productization/handoffs/feishu-passive-message-event-gate-handoff.md` |
| 九项剩余任务 completion audit | 已新增总审计 gate，把非 @ live 投递、单监听、first-class routing、权限负例、review DM/card、Dashboard auth、clean demo DB、Cognee/embedding 长期服务和 no-overclaim 映射到具体证据；当前默认可用日志重跑仍返回 `goal_complete=false`，blockers 为非 @ 群消息 live 投递、first-class 工具扩样、第二真实用户拒绝、真实 `/review` DM/card E2E 和 Cognee/embedding 长跑证据 | `scripts/check_openclaw_feishu_productization_completion.py`、`tests/test_openclaw_feishu_productization_completion.py` |
...
| Feishu live evidence run preflight | 已新增非发送型 live run 预检：检查当前单监听 owner 和 group-message event scope，生成四类日志路径、人工消息步骤、operator checklist、Feishu console remediation guide、packet 命令和 completion audit 命令；2026-05-03 read-only run 已写出 `logs/feishu-live-evidence-runs/20260503T111326Z/operator-checklist.md`，单监听通过但 event subscription fail closed，原因是 schema 缺 group-message scope；当前机器只看到 `openclaw-gateway`，因此计划 listener 应保持 `openclaw-websocket`，不要再启动 lark-cli listener 抢同一个 bot | `scripts/prepare_feishu_live_evidence_run.py`、`scripts/check_feishu_event_subscription_diagnostics.py`、`tests/test_prepare_feishu_live_evidence_run.py`、`tests/test_feishu_event_subscription_diagnostics.py` |
